...
**这两个新目录尚未出现在本地工作区**；以下重算使用终端打印的六位小数，
没有宣称核对新 CSV/cubin、完整源码指纹、GPU UUID 或计时期间时钟。

两次顺序均为 AB/BA/AB/BA/AB/BA/AB（A=baseline，B=cache），各版本每轮
都验证通过；每次六组边界各运行两次，共 24 次边界启动通过。两个尺寸的
...
**1.011986× / 1.012452×**，8192 为 **1.019976× / 1.019953×**。
