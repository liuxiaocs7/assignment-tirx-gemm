...
实际运行 `c6a92ff`，Slurm 27770 / step 0，GPU 778768b4…，CPU affinity
210–239；是同一张 B300 上的新作业。session 的 dirty 仅是新结果目录。
11 项执行源码哈希均匹配 `c6a92ff`，28 份 builder/编译捕获的源码哈希自洽，
没有额外 CUDA 文本转换。正式 baseline 的 CUDA、cubin、资源、NVRTC 版本及
选项与上轮 ready 的 baseline 逐字节一致，cubin SHA256 仍为
`05e01627056fa09d6c0c9ce44c94b79aaaca374a71db3d473c3e605df0937900`。

### 数值通过，候选性能全部失败

四个版本的评分形状通过数值校验，三个候选各八组边界、各运行两次，24 份
verification.json 均为 verified=true / launches=2 / timed=false。八轮四版本
在各个位置各出现两次，任意两版本的先后各四次。采集和管道退出码均为 0。
按原始 [samples.json](results_b300/step10_granularity.nVKp6r/step10_4096/samples.json)
和原评分阈值重建的四行 summary 与归档完全一致。

| 配置 | 中位数 ms | 最大值 ms | 对 baseline 配对耗时增加 | 更快轮数 | 达标样本 |
|---|---:|---:|---:|---:|---:|
| baseline：K64 / 四级 / SW128 | 0.138956 | 0.139820 | — | — | 6/8 |
| tmem_k64_sw64 | 0.178131 | 0.179228 | **28.155%** | 0/8 | 0/8 |
| tmem_k32_depth8 | 0.178197 | 0.178638 | **28.264%** | 0/8 | 0/8 |
| tmem_k32_depth10 | 0.179044 | 0.179789 | **28.716%** | 0/8 | 0/8 |

配对耗时增加是各轮 `(候选/对照 - 1)` 的中位数。终端 `paired_speedup=0.780`
表示吞吐约为 baseline 的 78%，不能写成“耗时只增加 22%”。这里耗时增加约28%。
候选最小值也明显高于 baseline 最大值，没有进入独立复测或正式采用的理由。

baseline 第 1、5 轮分别为 **0.139819733 / 0.139442126 ms**，超过原门槛
**0.719733 / 0.342126 μs**；最慢余量为 **−0.517%**。中位数虽然 PASS，
但距门槛仅约 **0.144 μs / 0.103%**。不能将这轮 6/8 或不同目录的通过轮数
变化解释为稳定性已修复。没有本轮全量 pytest / 全步骤验收。

### 对照链具体排除了什么

1. **退化在改变输入布局时已经出现。** K64、四级、网格、输入容量和运算量
   均保持不变，SW128 改 SW64 后耗时增加 28.155%，8/8 更慢。变化也包含布局
   所需的二维→三维 TMA 描述符与 MMA 地址参数，不能只归因于某一个硬件开关。
2. **细分输入没有补回损失。** K32/八级相对 K64/四级/SW64 的配对比值为
   **0.998926×**，耗时增加约 **0.108%**，只有 3/8 更快；在此 SW64 路径上
   基本持平。不能把相对正式版的整个 28% 退化说成“小块请求翻倍造成的”。
3. **再增加预取容量也无益。** K32/十级相对 K32/八级为 **0.995411×**，
   配对耗时增加约 **0.461%**，只有 1/8 更快。总动态 SMEM 从181248增至
   222208 B，没有得到有用收益；不继续盲目增加级数。

这批证据足以否定这三个具体候选，不证明所有 K32 布局都慢，也不证明输入
等待完全没有优化空间。此轮没有逐候选硬件 profile，不能断言 bank conflict、
Tensor Core 取数带宽减半、缓存 miss 或时钟变化各占多少。对未来有用的约束是：
保留正式 K64/SW128 作为基准，不再把 SW64 和更深 K32 ring 当作未经验证的方向。

### 编译产物：资源未溢出，静态指令减少不等于工作减少

| 配置 | REG / STACK / LOCAL | 动态 SMEM B | 静态 UTMALDG / UTCHMMA / R2UR | LDTM / UTMASTG |
|---|---|---:|---|---|
| baseline | 112 / 0 / 0 | 181248 | 12 / 16 / 103 | 4 / 4 |
| K64 / SW64 | 112 / 0 / 0 | 181248 | 12 / 16 / 103 | 4 / 4 |
| K32 / 八级 | 112 / 0 / 0 | 181248 | 12 / 8 / 87 | 4 / 4 |
| K32 / 十级 | 112 / 0 / 0 | 222208 | 12 / 8 / 87 | 4 / 4 |

四版均无 LDL/STL；三个候选的 ptxas 日志明确为零 spill loads/stores。
不能将退化解释为寄存器溢出，前三版的动态 SMEM 也完全相同。
K32 每次循环只做两个 K16 MMA，K 迭代数由64变128，每 consumer 每 tile
仍做256个 MMA；静态 UTCHMMA 从16减为8不表示数学运算减半。
同理，UTMALDG 静态位置同为12，但 K32 的实际 TMA 请求数翻倍。
四份 cubin 均不同，确认候选实际生效。

**处理：三个候选均已测未采用，脚本仅保留供复现，不原样重跑。**
本轮只补充分析、实验状态与可重算表格；没有新 GPU 执行，也未修改正式内核、
计时器、阈值、GPU tests 或候选实现（仅模块说明更新）。
重新生成37个评分形状及45组对照的数据表，旧40组对照行保持不变，新增五组
含直接对照和对正式版的总效果；30份输入文件的哈希已登记。重复生成产物一致，
候选模块去掉docstring后的AST一致，文档文件链接和`git diff --check`通过。
本轮未重跑248项工具测试；该数目属于准备实验时的验证记录。

### 当时的输入粒度实验设计（现已完成）

此前最近的 `step10_ready.FAnkEQ` 基本持平，随后实验由
[probe_step10_granularity.py](probe_step10_granularity.py) 生成隔离候选，入口为
[run_step10_granularity.sh](run_step10_granularity.sh)，具体运行/筛选说明见
[RUNNING.md](RUNNING.md#已完成输入分块粒度与缓冲容量全部不采用)。
正式 kernel、评分阈值、GPU tests 和计时器均未改动。

选择依据：当前路径的 MMA 等输入占角色时长 71.66%，TMA 等槽释放占 81.44%，
说明存在耦合等待，不能单凭任一比例断言显存带宽瓶颈。K128/两级已经退化，
四级到五级只有约 0.313% 配对收益；拆分就绪通知约 0.049%，不足以采用。
此前没有测过**缩小单份输入并保持总预取容量**。这是待验证假设，不是根因结论。

按优先级区分三个预测：

1. 若更细的到达/释放粒度能减少供数气泡，K32/八级应胜过相同 SW64 布局、
   相同总缓冲容量的 K64/四级；若通知/请求开销占主导，则会退化。
2. 若 K32 的八级仍不足以隐藏延迟，十级应在相同布局、粒度下进一步改善；
   不改善则不继续盲目加深。
3. 若主要差异来自布局，K64/四级仅换 SW64 就应有收益，而 K32 未必额外获益。

比较链是 baseline → `tmem_k64_sw64` → `tmem_k32_depth8` →
`tmem_k32_depth10`。所有版本都同时报告对正式 baseline 的配对比值。
“等容量”不等于所有其他成本相同：K32 在 4096 时每 tile 由 64 次 K 迭代变为
128 次，每 CTA 的 TMA 请求由 192 次变为 384 次；每 consumer 的 K16 MMA
总数仍为 256，输入字节量和数学运算量不变。K64/SW64 采用两片 K32 平面的
三维 TMA 描述符，K32/SW64 则是二维描述符；分块产生的代码生成差异需结合
GPU 归档的 SASS、寄存器及资源日志分析，不能将净收益只归因于某条 barrier。

基线及两个等容量版本的动态 SMEM 均为 181248 B，十级为 222208 B，低于
232448 B 预算。更多 barrier 位于现有 1024 B 头部，ready/free 槽不重叠；
各输入槽仍等两个 consumer 完成后才能覆盖。两 CTA 的期望字节在 K32 为
40960 B，在 K64 为 81920 B。TMEM 四个 ready/free 槽仍各有完整的两 CTA
读回保护。输入分块不改变累加顺序中的 K16 步长。

本地检查使用真实 TVM 0.26 的 SM100a/SM103a CUDA 生成，核对张量描述符、
TMA/MMA 共享内存地址、完整 K 覆盖、所有输出坐标、phase 跨 tile 推进、
延迟异步传输下的输入释放及 TMEM 复用；错误字节数/过早释放的变异应被测试抓住。
GPU 数值边界为 `(4096,3072,K)`，K=64/192/256/320/384/512，外加
`(4608,2560,320)`、`(512,9728,192)`，各验证两次。小网格单槽及宽 N
分支的生成 CUDA/张量描述符逐字保持正式版。源码生成和模拟均不能替代 B300
数值、同步及性能测试。准备阶段尚无该候选 GPU 数据；现已回传，见本节开头。

新增及受影响本地检查共 **248 passed，零跳过**：粒度/runner 53 项通过后，
补强 K 循环和 acquire 顺序断言，粒度 35 项再次通过；ready、epilogue、
双缓冲、persistent 和已有 input probe 195 项通过。248 是不同测试用例数，
不重复累计重跑的 35 项。`bash -n`、`git diff --check` 通过；正式内核哈希
仍为 `727a44403b6c1ea0a0499c8212e5796d0fa2b31944929e9ecd50d60be4b961f2`。

## 最新输入通知实验：基本持平，未解决性能余量，不采用

`77115e7` 归档 [step10_ready.FAnkEQ](results_b300/step10_ready.FAnkEQ/)。
实际执行 `f392bf1`，Slurm 27690 / step 0，GPU 778768b4…，CPU affinity
210–239，与前轮写回实验是同一作业、同一卡。session 中 dirty 仅来自新结果目录；
核对内核、utils、benchmark、probe 与输入通知模块的执行版本哈希均一致。
正式内核 SHA256 仍为 `727a44403b6c1ea0a0499c8212e5796d0fa2b31944929e9ecd50d60be4b961f2`。
baseline CUDA、cubin、资源、NVRTC 版本与选项逐字节匹配首次正式验收 vM3h3I；
共同 cubin SHA256 为 `05e01627056fa09d6c0c9ce44c94b79aaaca374a71db3d473c3e605df0937900`。

### 已完成验证，性能依然贴线

baseline 与候选的 4096 数值检查通过，候选八组短 K/矩形/跨 tile 边界各验算两次。
八份 verification.json 均为 verified=true、launches=2、timed=false；采集
退出码与 probe/tee 均为 0。没有本轮全量 pytest 或全步骤 benchmark，不能将
这份单项 probe 写成新的全量验收。

[原始样本](results_b300/step10_ready.FAnkEQ/step10_4096/samples.json)严格按
AB/BA 各四轮，两个版本在每个位置各四次。使用原 timer，warmup=10、repeat=30。
表中每个样本是该轮 30 次执行的平均耗时，不是单次 kernel 延迟的分位数。

| 配置 | 中位数 ms | 最小 ms | 最大 ms | 快于 baseline | ≤0.139100 ms |
|---|---:|---:|---:|---:|---:|
| 正式 baseline | 0.139018 | 0.138739 | 0.139427 | — | 5/8 |
| tmem_split_ready | 0.138973 | 0.138465 | 0.139199 | 5/8 | 6/8 |

逐对 `baseline/候选` 中位数为 **1.000491993×**，终端保留三位小数显示为
**1.000×**；逐对耗时下降中位数为 **0.049167%**，节省 **0.068267 μs**。
两个总体中位数相除则为 1.000322357×（约 0.032%），属于不同统计量，不混用。
八轮逐对节省微秒为：

```text
+0.348806, -0.459735, -0.007470, +0.107733,
-0.028801, +0.028801, +0.741323, +0.262403
```

正数表示候选快；5/8 更快、3/8 更慢，没有足够的一致收益。按先后拆分，候选
后测为 2/4 更快、配对 1.001228×，先测为 3/4 更快、配对 1.000492×；每组
只有四对，也不能拿这些小样本证明稳定优势。

候选第 2、8 轮分别为 **0.139198939 / 0.139164797 ms**，仍超过原门槛；
最慢轮超出 **0.098939 μs / 0.0711%**。其中位数虽低于门槛，但余量只有
**0.127201 μs / 0.0914%**。6/8 比 5/8 多一次通过，不能解释为可靠通过率
提升，也不能把这八个批量样本外推成以后 pytest 的成功概率。

**结论：基本持平、没有解决余量，候选不采用。** summary 的两个 PASS 只判断
各自中位数，退出码 0 只表示采集完成。当前正式配置继续保留；不需原样重复
此候选，也不将前后不同目录 baseline 从 2/8 到 5/8 的变化记作优化效果。

### 假设与实际编译结果怎样对应

原版两计算组共同等待 B、A0、A1 全部到齐；候选改为各等共享 B 与自己的 A。
这既可能减少不必要等待，也增加每阶段的通知和 acquire。当前净收益接近零，
所以“拆开通知足以产生有用加速”的假设未获支持。它不证明输入等待完全没有
成本，也不证明两份 A 永远同时到齐；本轮没有逐候选的等待时间 profile。

两版资源相同：**REG112、STACK0、LOCAL0，动态 SMEM181248 B**，SASS 均无
LDL/STL。保留同样的数据缓冲、两个共享 B consumer 和完整数学运算。
有一个值得记录的编译差异：baseline 的 MMA 循环自动展开四个 K64 阶段，
候选展开两个；静态 UTCHMMA 位置数 **16→8**。两版生成 CUDA 都仍是 64 次
K64 迭代、每次四条 K16 MMA，运算总量没有减半。SASS 的循环计数增量分别
为 4 和 2，回跳区间为 baseline `0x16f0–0x2280`、候选 `0x19a0–0x2050`。
输入 UTMALDG / 写回 UTMASTG / 读回 LDTM 的静态位置数都仍为 12 / 4 / 4。

因此，这轮测量评估的是拆分通知经过实际编译后的整体效果；不能把约 0.05%
差异精确归因于某个 barrier，也不能把静态指令数减半当作计算量或耗时减半。
保留原始结果和未采用的源码，避免下一轮把它当成未经尝试的新优化。

本轮离线重算原始数据并同步实验状态，正式 kernel、计时器、阈值与 GPU tests
均未修改。数据表已包含 40 组显式对照；后续软件优化仍需在原计时方式下争取
数微秒余量，不能靠采用本轮约 0.068 μs 的微小差异宣称问题修复。

## 最新写回实验：三个候选均退化，不采用

`bbfa1bf` 归档 [step10_epilogue.xVWC2d](results_b300/step10_epilogue.xVWC2d/)。
实际执行 `98f2e08`，Slurm 27690 / step 0，GPU 778768b4…，CPU affinity
210–239。工作区只多了结果目录；正式内核 SHA256 仍为
`727a44403b6c1ea0a0499c8212e5796d0fa2b31944929e9ecd50d60be4b961f2`。
核对执行版本的内核、utils、benchmark、probe 及写回模块哈希全部一致。
baseline CUDA、cubin、资源报告、NVRTC 版本和选项与首次正式验收 vM3h3I
逐字节一致；cubin SHA256 仍为 `05e01627056fa09d6c0c9ce44c94b79aaaca374a71db3d473c3e605df0937900`。

### 数值、采集与性能分别判断

四个 4096 构建先通过数值验证，三个候选各六组边界验证两次；18 份
verification.json 均为 verified=true、launches=2、timed=false。全部采集
退出码及 probe/tee 管道退出码为 0。这说明实验采集完整，不代表性能达标。

重算 [samples.json](results_b300/step10_epilogue.xVWC2d/step10_4096/samples.json)：
八轮中每个版本在四个位置各出现两次，每对先后各四次。所有百分比取同轮
`候选耗时 / baseline 耗时 - 1` 的中位数，微秒增加也取逐对差的中位数。

| 配置 | 中位数 ms | 配对耗时增加 | 配对增加 μs | 快于 baseline | ≤0.139100 ms |
|---|---:|---:|---:|---:|---:|
| 正式 EPI32 单缓冲 | 0.139205 | — | — | — | 2/8 |
| tmem_epi64 | 0.141452 | 1.614% | 2.246 | 0/8 | 0/8 |
| tmem_epi128 | 0.143390 | 2.978% | 4.149 | 0/8 | 0/8 |
| tmem_epi32_double | 0.140787 | 1.120% | 1.560 | 0/8 | 0/8 |

baseline 中位数比门槛高 **0.105 μs / 0.076%**，最慢样本高
**0.515 μs / 0.370%**。这次没有解决贴线问题，也不能以程序退出码 0
宣称验收通过。三个候选在两个测量先后方向均落后，当前结论是**全部不采用**。
无需原样重复这三个写回实验。

### 指令确实减少，为什么仍然慢

这里的 epilogue 指最终结果从 TMEM 读出、转成 FP16、经过 SMEM 写回显存。
原版把每个 consumer 的 128 列分成四次 32 列；两种加宽候选合并写回，
双缓冲候选尝试重叠上一块搬运与下一块准备。三个候选都保留四级输入、
共享 B、原 MMA/TMEM 槽位和网格，避免混入减少输入深度等因素。

| 配置 | REG | STACK / LOCAL | 动态 SMEM B | 静态 UTMASTG 指令位置 |
|---|---:|---:|---:|---:|
| baseline | 112 | 0 / 0 | 181248 | 4 |
| epi64 | 105 | 0 / 0 | 197632 | 2 |
| epi128 | 100 | 0 / 0 | 230400 | 1 |
| epi32_double | 128 | 0 / 0 | 197632 | 4 |

四份 SASS 都没有 LDL/STL；输入 UTMALDG、MMA UTCHMMA、TMEM LDTM 的静态
位置数均为 12、16、4。这里是二进制指令位置数，不是全网格动态执行次数。
更少的写回提交已被编译器实现，寄存器在加宽方案中也确实下降，但耗时仍增加。
因此不能解释成“加宽导致寄存器溢出”，也不能把共享内存增加直接说成驻留数下降：
原版在当前布局下就已受 SMEM 限制为每 SM 一个 CTA，加宽后仍不能容纳两个。

与已有角色 trace 一起看，写回角色 86.19% 时间在等 MMA，epilogue 仅占该角色
12.27% 且与计算重叠。减少写回步骤，未必缩短决定总耗时的计算链。更大的
SMEM 分片、不同搬运批次及编译器指令调度可能抵消收益；本轮没有逐变体 profiler，
**不能进一步确定各因素分别贡献多少**。可确认的是这三种具体改法整体更慢。

## 输入通知实验设计（已完成，结果见本文开头）

用户提供的队列中仅自身作业运行，后续直接推进软件优化，不要求重新申请
同一 GPU 或先做管理员排查。`probe_step10_ready.py` / `run_step10_ready.sh`
增加一个独立候选 `tmem_split_ready`，直接对照正式 baseline。

原来一个 K64 阶段的 B、A0、A1 共用同一通知，两个 MMA consumer 都要等
**81920 B** 全部到齐。实际上 consumer 0 只读 B+A0，consumer 1 只读 B+A1。
候选让 B 单独通知 **16384 B**，A0/A1 各通知 **32768 B**；均计入两 CTA。
B 每 CTA 仍只搬一次，各 consumer acquire B 和自己的 A 后，执行原 fence/MMA。
输入槽的释放通知仍要求两个 consumer 的 MMA 完成，不能提前覆盖共享 B。

预测：若一个 consumer 被另一份尚未到齐的 A 阻塞，分开通知有机会提前开始
计算。反预测：若两份 A 几乎同时到齐，或主要受 Tensor Core 执行限制，每阶段
多两个 expect_tx、多一个等待可能只增加成本。本轮只改变通知粒度，不叠加
五级输入、写回、网格或共享 A，便于判断这个假设。

新增八个 A-ready barrier 使用原先预留的前 1024 B，最高用到第 24 个
uint64 槽（末尾 200 B），旧 barrier、A/B/D 数据地址均不移动，动态 SMEM
仍为 **181248 B**。相位沿原四槽 ring 跨输出 tile 推进，TMEM 双槽交接不变。
候选只作用于原规则选出的窄 N persistent 双槽分支，其余形状生成原 CUDA
与 TMA 描述符。正式 gemm_kernels.py、计时器、阈值和 GPU tests 均保持。

本地检查生成 CUDA 的每个通知地址、期望字节、两 CTA 输入完成和两 consumer
释放条件，以及 MMA/写回/调度/相位后缀等价。随机事件模型延迟各 CTA 的输入
完成与各 consumer 的计算完成，覆盖七个连续输出 tile，并用遗漏 A acquire、
错误字节数、提前释放的故障变体确认检查可以失败。这些不能替代 B300 数值验证。

本地新检查及受影响回归 **207 passed，177.08 s，零跳过**，覆盖新输入通知、
两份 shell runner、当前/历史写回、persistent 工具和当前输入 ring。Bash 语法、
Python 编译、CLI help、文档链接和 diff 空白检查通过；37 形状/39 对照的数据表
可从原始记录重复生成。macOS 本机未执行候选 NVRTC/cubin 或 GPU 数值/计时。

以下命令已完成，保留供复现，当前不需要原样重复：

```bash
bash run_step10_ready.sh
```

脚本先验算 baseline/候选 4096；候选验证八组边界，每组两次，包括 K64/128/
192/256/320/384 的跨 tile 复用、部分 L2 分组和单波/多波边界。全部验证通过
后才用原 timer、warmup=10/repeat=30、八轮 AB/BA 交替计时。保存 UUID、
源码/模块哈希、builder、CUDA/cubin、资源、原始样本和退出码。目标仍是原计时
下约 ≤0.135 ms 留出余量，固定评分门槛仍为 0.139100 ms。实际回传结果基本
持平、仍有两个样本超线，候选不采用；详细统计见本文开头。

## 单卡诊断已完成：Graph 有小幅改善，负载期间 SM 采样为 1095 MHz

`5da9d1f` 归档了 [step10_runtime._v5qw220](results_b300/step10_runtime._v5qw220/)
与 `step10_diagnose.9pEDv4`。runtime 实际执行 `6ad3f6d`，仍为 Slurm
27689 / step 0、GPU 778768b4…、CPU affinity 210–239。本地核对四个核心
源码哈希和 probe 哈希均匹配；CUDA、cubin、NVRTC 版本/选项及资源报告与
MBawzh、首次正式验收 vM3h3I 分别逐字节一致。共同 cubin SHA256 为
`05e01627056fa09d6c0c9ce44c94b79aaaca374a71db3d473c3e605df0937900`。

### 1. 同一内核换提交方式，改善约 2.2%–2.5%，普通路径仍未稳定达标

重算 [before_load.json](results_b300/step10_runtime._v5qw220/before_load.json)、
[after_load.json](results_b300/step10_runtime._v5qw220/after_load.json) 与
[summary.json](results_b300/step10_runtime._v5qw220/summary.json)：每阶段八对
样本，AB/BA 完整平衡，32 个计时结果均记录数值校验通过。两次污染输出后的
Graph replay 也通过，说明本环境的捕获/回放已实际运行成功。

| 阶段 | 普通提交中位数 ms | 普通样本在原门槛内 | Graph 中位数 ms | 配对普通/Graph | 配对耗时下降中位数 |
|---|---:|---:|---:|---:|---:|
| 持续负载前 | 0.139833069 | 0/8 | 0.136542400 | 1.025504× | 2.4869% |
| 持续负载后 | 0.139888000 | 2/8 | 0.136803738 | 1.022026× | 2.1551% |

Graph 在 **16/16** 对中更快，合并配对耗时差中位数约 **3.151 μs**。
这支持提交方式相关开销/状态有影响，不能把差值全部视为 Python 时间；Graph
还改变发射密度、驱动调度等。与历史约 0.094 ms 相比，Graph 仍约慢 45%，
因此此次提交方式改善未恢复历史速度。历史比较非同环境配对，不是优化收益。

负载后普通路径中位数仅比负载前增加 **0.0393%**，没有持续改善。八秒负载
没有解决当前重复失败，不能靠延长预热或只选两次通过样本宣称修复。Graph 的
计时口径与评分不同，其数值低于门槛也不计作正式 PASS。退出码 **1** 正确表示
采集完整而普通路径存在慢样本，监控 stderr 为空。

### 2. 新证据来自持续负载，不再只是进程退出后的空闲快照

[telemetry.csv](results_b300/step10_runtime._v5qw220/telemetry.csv) 有 80 个约
100 ms 间隔的采样。结合 [负载区间](results_b300/step10_runtime._v5qw220/load_intervals.json)：
四块普通→Graph→Graph→普通负载合计 **8.0083 s**，不是正式计时区间。

- 80/80 为 **P0、SM 1095 MHz、显存 3996 MHz**，两种提交方式都如此。
- 开头三个利用率读数为 0%，其余 77 个为 **99%–100%**；监控利用率有采样
  窗口，不能将 100% 解读为 Tensor Core 达到理论峰值。
- 温度 **36–52°C**；功耗读数由 266.19 W 上升，最大 **860.38 W**；显示功率
  上限为 **1100 W**。这些采样不能排除瞬时或设备内部功耗管理约束。
- [负载前](results_b300/step10_runtime._v5qw220/gpu_before_load.txt)与
  [负载后](results_b300/step10_runtime._v5qw220/gpu_after_load.txt)的 SW Power
  Capping 累计值从 **61957234300 μs** 增至 **61965395790 μs**，新增
  **8161490 μs = 8.161490 s**。快照区间还包含监控启停与校验等，不把该数
  精确除以 8.0083 s 当作占比。SW/HW Thermal、HW Power Braking、Sync Boost
  计数均未增加。两端当前原因显示 Not Active 不否定区间内累计计数的增量。

这使运行时功耗/频率管理成为优先核对方向；记录确实观察到负载期间的
1095 MHz 与新增功耗限制。仍未证明人为锁频、GPU 故障或特定策略就是所有失败
的根因。Applications/Max Clocks 的 2032 MHz 不是保证达到的工作频率；此处
普通/Graph 短计时区间未被连续监控，不能将持续负载的所有状态直接套用到它们。

### 3. 当时建议核对设备策略；当前按用户要求推进软件优化

这轮对照已完成，不必原样重复。将上述 UUID、1095 MHz 高利用率采样、
8.161490 s 限功耗增量及两个快照交给集群管理员，核对锁频/boost、实际功率
管理与 power smoothing 策略；快照的 Power Smoothing 显示 Insufficient
Permissions，不能由用户态数据确定配置。应由管理员确认是否存在异常策略，
不要从 Max Clocks 直接推导应强制设为 2032 MHz。

当前软件优化直接以此环境为基线，不以管理员核对作为前提。
从本轮普通路径最慢 **0.140673 ms** 降至 **0.135 ms** 需约 **4.03%**；
继续针对正式共享 B 内核优化，并用原计时器验收。Graph 的收益不是内核改动，
不能直接替代评分路径。若要进一步分清 3 μs 差值来自何处，再采 GPU 时间线；
目前尚无该时间线，不能声称已经测出了每次 Python 提交等待。

本次仅重算归档数据、核对源码与二进制并更新文档，没有本地 GPU 重跑或内核修改。

## 单卡诊断设计与此前状态（已由上节结果更新）

用户确认当前只能使用一张 GPU；不能把重新申请 Slurm 作业当成换卡实验，也不
以取得另一张 GPU 作为继续工作的前提。历史记录中的不同 UUID 仍是历史事实，
但不能推断用户现在可申请到那些设备。

当时回传的 `step10_diagnose.9pEDv4` 先按终端输出记录，现已由 `5da9d1f`
归档并核对七个源码哈希、采集退出码及原始 CSV；执行
...
设计阶段仅增加诊断工具与 CPU 工具测试，**当时未在 B300 运行新实验**，未改正式
...
实际执行提交 `8cbdf2e`，Slurm **27689 / step 0**，CUDA 逻辑设备 0 对应 UUID
**778768b4-6c9e-e483-890e-0812760948ae**。用户另确认此前两次失败在同一张
GPU、不同作业；不能把不同作业视为不同物理卡。核心四文件、Step 10 测试和
诊断工具哈希均与执行提交/当前文件匹配；`dirty=True` 来自 session 中记录的
未跟踪结果文件，没有已跟踪源码修改。

| 运行 | 耗时 ms | 超过 0.139100 ms | 结果 |
|---|---:|---:|---|
| 单项 pytest 1 | 0.139978 | 0.631% | 性能断言失败 |
| 单项 pytest 2 | 0.140633 | 1.102% | 性能断言失败 |
| 单项 pytest 3 | 0.140726 | 1.169% | 性能断言失败 |
| 正式 benchmark 八轮中位数 | 0.139725335 | 0.450% | SLOW，2/8 样本 PASS |
| 正式 benchmark 最慢样本 | 0.140390396 | 0.928% | SLOW |

三个单项测试均先完成数值验证，失败在 `check_timing`；benchmark 也经过数值
校验。两个通过样本为第 4/5 轮 **0.136765862 / 0.137114668 ms**，其余六轮超线。
AB/BA 各有 1/4 样本通过，kernel-first / cuBLAS-first 的 kernel 中位数分别为
**0.139912534 / 0.139579201 ms**，两种顺序均超线。问题不依赖先执行 Step 1–9，
也不是只有原先单轮计时才出现；当前条件下已经是可重复的性能不足。

独立执行 `python3 tools/summarize_benchmark_stability.py
results_b300/step10_diagnose.MBawzh/step10.csv --expected-trials 8`，重算为
**2/8 PASS，退出码 1**。三次 pytest、benchmark、summary 的命令退出码均为 1，
所有 tee 和设备采集退出码为 0，故综合退出码 1 是性能失败，不是采集工具错误。
cuBLAS 八轮中位数 **0.130181332 ms**，逐对 `cuBLAS/kernel` 中位数为
**0.932978×**；本轮自研耗时逐对增加约 **7.18%**，不能沿用旧“领先 cuBLAS”结论。

### 二进制相同，先区分设备与运行状态

本次 `module_01.cu`、`module_01.cubin`、NVRTC 选项、NVRTC 版本与资源报告，
分别和以下三份 4096 baseline 逐字节一致。共同 cubin SHA256 为
`05e01627056fa09d6c0c9ce44c94b79aaaca374a71db3d473c3e605df0937900`，
资源报告均为 **REG112、STACK0、LOCAL0**。这排除了这些归档之间的生成内核/
编译设置差异，尚不排除宿主调度、驱动运行状态或硬件状态差异。

| 归档 | GPU UUID 前缀 | baseline 中位数 ms |
|---|---|---:|
| 首次正式验收 `step10_final.vM3h3I` | dadf9f3b… | 0.094018 |
| 五级对照 `step10_depth5_balanced.Fm3CcJ` | 778768b4… | 0.139042 |
| 几何对照 `step10_geometry.PywrmG` | dadf9f3b… | 0.109001 |
| 本次 `step10_diagnose.MBawzh` | 778768b4… | 0.139725 |

这里快慢记录与物理设备相关，值得受控复核，但不是“某卡损坏”或固定性能差异
的证明：采集时间、作业状态也不同。`9e3b989` 五轮全量验收没有记录 GPU UUID，
因此不能把那五轮未经验证地归到 dadf9f3b…；上表用的是有 UUID 的首次验收。

### 本次设备快照的确切含义

五次快照温度均 **34°C**，当前功率上限 **1100 W**，没有列出的计算进程；
它们在进程结束后采集，不能证明运行期间没有其他负载。当前 SM 时钟均为
**1095 MHz**，Applications/Max Clocks 为 **2032 MHz**；后者不是本次计算
实际达到的时钟，前者也不是 kernel 内时钟测量。

SW Power Capping 从开始到第三次 pytest 后均为 **61956716468 μs**，
benchmark 后变成 **61956894658 μs**，增加 **178190 μs（178.190 ms）**。
SW Thermal Slowdown 始终 **124495234 μs**；HW Thermal、Sync Boost、
HW Power Braking 计数始终为 0。结论是：**三次 pytest 所覆盖区间没有新增的
这些限频计数，benchmark 所覆盖区间有新增功耗限制，未见新增热限制。**
不能将历史累计热限制当作本次过热，也不能用 benchmark 的功耗限制解释前三次
计数不变时的失败。benchmark 窗口约 5 s，包含编译、校验、预热、自研及 cuBLAS；
两者 CUDA-event 计时批次合计仅约 **64.60 ms**，178.19 ms 不能全归到计时内核。
...
若需要在 778768b4… 上解决问题，下一步应与管理员核对工作负载下的实际频率/
boost 策略，并核对作业 CPU 调度；只有受控数据支持后才调整运行环境。另一条
路线是在此较慢条件下继续优化同轮 baseline，保留原评分协议。以本次最慢
pytest 为起点到 **0.135 ms** 需约 **4.07%** 降时，既有五级约 0.31% 信号
不足以预先承诺解决问题。保持原共享 B 结构，不重新采用已排除的 N64 候选。
这些后续对照尚未执行，当前不宣称根因已经确定或性能已经修复。

## 最新复测：32d4fa8 再次出现 Step 10／4096 性能失败

2026-09-15 用户回传 `results_b300/validate_32d4fa8.ABza4T` 的完整终端输出：
**61 passed / 1 failed，80.72 s**。唯一失败是 Step 10／4096 的计时断言，
耗时 **0.139973 ms**，超过原门槛 **0.139100 ms** 约 **0.873 μs / 0.628%**。
该用例的数值验证在计时前已通过，新增五项分支数值测试也通过。
随后另一份回传摘要仍是 **61 passed / 1 failed，80.40 s**，同一用例
**987.70 TFLOP/s**，据打印精度反算约 **0.139151 ms**，超线约 **0.036%**；
后者没有给出精确耗时、目录和版本指纹，不能当作完整原始记录。

后续 `47f55d7` 已归档 `pytest_32d4fa8_2.log`，与上述 80.40 s / 987.70 TFLOP/s
摘要一致，打印耗时为 **0.139151 ms**。同批另外四份全量日志与 ABza4T 一起
共六份记录，其中两份通过、四份在 Step 10／4096 性能失败；它们没有完整的
作业/设备元数据，不能将 2/6 当作估计未来通过概率的独立随机样本：

| 全量日志 | Step 10／4096 ms | 总结 |
|---|---:|---|
| `pytest_32d4fa8_1.log` | 0.138625 | 62 passed |
| `pytest_32d4fa8_2.log` | 0.139151 | 61 passed / 1 failed |
| `pytest_32d4fa8_3.log` | 0.139110 | 61 passed / 1 failed |
| `pytest_0915_4.log` | 0.140086 | 61 passed / 1 failed |
| `pytest_0915_5.log` | 0.139067 | 62 passed |
| `validate_32d4fa8.ABza4T/pytest.log` | 0.139973 | 61 passed / 1 failed |

其中 0.139067 ms 虽然通过，只比门槛低约 **0.033 μs**，不构成可靠性能余量。

**先前五轮验收通过仍然成立，但后续运行下稳定满足原性能门槛的问题尚未解决。**
“已有验收记录”和“当前每次运行都可靠通过”是两个结论，不能用前者覆盖这两次失败。
保留旧 P1 关于缺少验收证据的关闭结论，同时将当前性能稳定性作为未解决问题记录。

下表比较第一份新输出与五份已归档 pytest 的中位数；比较口径均为 pytest，
不混用 benchmark 时间，也不把比值当作版本改动的收益：

| 用例 | 旧五轮 pytest 中位数 ms | 新输出 ms | 耗时增加 |
|---|---:|---:|---:|
| Step 3／2048 | 0.053472 | 0.088818 | 66.10% |
| Step 4／2048 | 0.020834 | 0.035143 | 68.68% |
| Step 6／4096 | 0.155654 | 0.248172 | 59.44% |
| Step 8／4096 | 0.117104 | 0.161298 | 37.74% |
| Step 9／4096 | 0.098727 | 0.142851 | 44.69% |
| Step 10／4096 | 0.093911 | 0.139973 | 49.05% |
| Step 10／8192 | 0.651157 | 0.872601 | 34.01% |

多个步骤一起变慢，且接近此前 `pytest_3a9d486_2.log` 的较慢记录；这支持先检查
共同运行条件，但不能仅凭耗时确认限频、资源竞争或编译差异中的哪项是原因。
用户的 git status 只有三个未跟踪结果目录，没有已跟踪文件改动；本地也确认
`9e3b989..32d4fa8` 的正式内核、计时评分代码与 GPU tests 一致。后续 `47f55d7`
已归档该次 `pytest.log` 和 `version.txt`，核对为 `32d4fa8`、当前内核哈希；
该次全量没有设备状态证据。其余两个 validate 目录仅有版本文件，不能推断测试结论。

原复测脚本使用 `set -euo pipefail`。pytest 管道非零后 Bash 立即退出，后续
全步骤 benchmark 未执行；本次不能写成“pytest 与 benchmark 都已复测”。
新增 [run_step10_diagnose.sh](run_step10_diagnose.sh) 固定执行三次该评分用例和
八轮正式 Step 10／4096 benchmark，失败后继续保留记录，保存命令与 tee 退出码、
设备 UUID、核心源码指纹、编译产物和计时外设备快照。
[逐样本汇总器](tools/summarize_benchmark_stability.py)直接读取原评分表，避免
中位数 PASS 掩盖个别超线；其诊断结果不替代全量验收或修改正式评分规则。
本机无 NVIDIA GPU；用户后续执行并归档的新诊断见上节，不宣称性能已修复。
新增入口在本地的 **13 项工具检查通过**，覆盖 pytest/benchmark/tee/设备采集
失败、失败后继续采集，以及原始样本异常或门槛不匹配时拒绝报告成功。
它们使用模拟 GPU 命令验证采集控制流，不计入 GPU 数值或性能通过次数。

## 当前验收：9e3b989 五轮全量 pytest 与全步骤 benchmark 均通过

用户补充的五轮结果已在 `0961ddc` 归档，本机逐份核对
`results_b300/pytest_9e3b989_{1..5}.log`、`benchmark_9e3b989_{1..5}.log` 与
`all_steps_9e3b989_{1..5}.csv`。**每轮全量 pytest 均为 62 passed，合计
310/310 次用例执行通过；五份正式全步骤 benchmark 每份 37 行，合计
185/185 PASS。可记录 `9e3b989` 在本次 B300 运行条件下验收通过，包括
Step 9 基址缓存采用后的回归。** 此前“最新全量 61 passed / 1 failed、Step 9
回归待执行”的状态已更新；历史失败继续保留，后续候选优化是可选工作。

| 轮次 / pytest 原始日志 | pytest 用时 s | 全步骤 CSV | Step 10／4096 ms | Step 10／8192 ms |
|---|---:|---|---:|---:|
| [1：62 passed](results_b300/pytest_9e3b989_1.log) | 82.04 | [37/37 PASS](results_b300/all_steps_9e3b989_1.csv) | 0.094295 | 0.648943 |
| [2：62 passed](results_b300/pytest_9e3b989_2.log) | 81.61 | [37/37 PASS](results_b300/all_steps_9e3b989_2.csv) | 0.094282 | 0.643915 |
| [3：62 passed](results_b300/pytest_9e3b989_3.log) | 81.84 | [37/37 PASS](results_b300/all_steps_9e3b989_3.csv) | 0.094378 | 0.642770 |
| [4：62 passed](results_b300/pytest_9e3b989_4.log) | 81.58 | [37/37 PASS](results_b300/all_steps_9e3b989_4.csv) | 0.094261 | 0.644471 |
| [5：62 passed](results_b300/pytest_9e3b989_5.log) | 82.00 | [37/37 PASS](results_b300/all_steps_9e3b989_5.csv) | 0.094725 | 0.649407 |

CSV 全部记录执行提交 `9e3b989cc50a8143718048351bcd79a1e1343d47`，B300
148 SM / SM103a、TVM 0.26.0、PyTorch 2.14.0+cu130、CUDA 13.0，NVRTC /
register-usage-level=10。每份为 **trials=1、warmup=10、repeat=30**：这是五次
完整运行，不是一次每尺寸七轮测试。所有原始样本按原门槛重算均与 PASS
字段一致；pytest 中 Step 10／4096 范围为 0.093828–0.094100 ms。

| Step / 尺寸 | 五次正式 benchmark 范围 ms | 原门槛 ms | 最慢样本余量 |
|---|---:|---:|---:|
| 9 / 1024 | 0.012174–0.012496 | 0.019500 | 35.92% |
| 9 / 2048 | 0.016701–0.016889 | 0.029900 | 43.52% |
| 9 / 4096 | 0.098299–0.098730 | 0.150800 | 34.53% |
| 9 / 8192 | 0.700297–0.703071 | 1.024400 | 31.37% |
| 10 / 1024 | 0.010786–0.010915 | 0.032500 | 66.41% |
| 10 / 2048 | 0.016580–0.016701 | 0.045500 | 63.29% |
| 10 / 4096 | 0.094261–0.094725 | 0.139100 | **31.90%** |
| 10 / 8192 | 0.642770–0.649407 | 0.946400 | **31.38%** |

185 行的 `gemm_kernels`、`utils`、`benchmark`、`benchmark_diagnostics` 四个
源码 SHA256 全部与执行提交及当前文件一致，内核为
`727a44403b6c1ea0a0499c8212e5796d0fa2b31944929e9ecd50d60be4b961f2`。
`9e3b989..f06d88f` 的这四个文件与 `tests/` 没有差异；新几何 probe 没有改动
正式评分路径。CSV 记录 `dirty=True`，但未单独保存当时的 git status；pytest
日志本身没有源码指纹，执行版本归属同时依据用户确认，不能说它独立记录了哈希。
五轮也没有单独的退出码、GPU UUID 或编译产物；验收依据完整 pytest 摘要和
带版本/核心源码指纹的正式 CSV，不声称额外核对了这些缺失文件。

五次进程每次只有一个 trial，实际 cuBLAS 顺序均为 `kernel → cuBLAS`，
没有形成跨 trial 的 AB/BA。本批可支持固定阈值验收，不能用来新增稳定领先
cuBLAS 的结论。历史慢分配上的超线也不因此被抹去。

## 几何实验已完成：PywrmG 三候选均慢于 baseline，不采用

`f06d88f` 保存完整 [原始样本](results_b300/step10_geometry.PywrmG/step10_4096/samples.json)、
[CSV](results_b300/step10_geometry.PywrmG/step10_4096/summary.csv)、
[元数据](results_b300/step10_geometry.PywrmG/step10_4096/run.json) 与编译产物。
实际执行提交为 `0961ddce53027646e552a90a0964d9a00713be1d`，Slurm job
27561 / step 0。内核哈希与上述验收一致。八轮为 ABCD/DCBA/CDAB/BADC 重复
两次，每个版本各位置两次、每对版本先后各四次；以下从未舍入样本重算。

| 配置 | 中位数 ms | 最大值 ms | 对 baseline 配对加速 | 配对耗时增加 | 达标样本 |
|---|---:|---:|---:|---:|---:|
| baseline：N128 / 64 clusters / 四级 | 0.109001 | 0.111588 | 1.000000× | — | 8/8 |
| `tmem_max_clusters`：N128 / 74 clusters | 0.113731 | 0.117585 | 0.954677× | **4.75%** | 8/8 |
| `tmem_n64`：N64 / 74 clusters / 四级 | 0.147771 | 0.150235 | 0.732603× | **36.50%** | 3/8 |
| `tmem_n64_depth5`：N64 / 74 clusters / 五级 | 0.135395 | 0.136641 | 0.805065× | **24.22%** | 8/8 |

三个候选相对正式 baseline 都是 **0/8 轮更快**。N64 四级相对直接对照
`max_clusters` 加速为 0.775871×，耗时增加 28.89%；五级相对 N64 四级为
1.098989×，耗时降低 9.01%，8/8 更快，但仍显著落后正式内核。
“1.099×”只表示五级改善了 N64 自身，不能据此采用 N64 五级。
按候选先/后测分组，三个直接对照加速分别为 0.954677/0.954680×、
0.776814/0.769630×、1.099370/1.098989×，结论在两种先后位置一致。

四个方阵构建验算通过，三个候选各八种边界两次，共 **48 次边界启动通过**；
每轮计时输出亦验算。八个运行源码指纹、28 份 builder/CUDA 指纹和候选变换
均核对通过，所有构建均有 NVRTC cubin；无 CUDA 后处理差异。device/probe/tee
退出码均为 0。baseline 的 CUDA、cubin、NVRTC 参数/版本与
`step10_depth5_balanced.Fm3CcJ` 对应四个文件逐字节一致。

4096 的实际寄存器为 baseline **112**、max_clusters **98**、N64 两种深度
均 **69**，全部 STACK/LOCAL=0。动态 SMEM 分别为 181248、181248、164864、
201728 B/CTA；资源报告的 `SHARED:1024` 是静态项，不能代替动态总量。
N64 单级 TMA 字节虽降到 73728，但任务翻倍使总请求字节约增到 N128 的
1.8 倍，MMA/调度次数也翻倍。结果否定了本轮几何的性能收益，说明减少寄存器、
增加 cluster 不自动提速；这些是成本解释，尚无该候选的硬件 profile 来量化归因。

GPU UUID 为 `dadf9f3b-df58-d3fa-07b0-5fe223423db1`，与前次 Fm3CcJ/L2 的
`778768b4-6c9e-e483-890e-0812760948ae` 不同；不能将此次约 0.109 ms 的 baseline
相对前轮约 0.139 ms 的变化归给未采用的候选。本轮 baseline 也从前几轮约
0.094 ms 变到后几轮约 0.111 ms。35 s 前后快照中 SW Power Capping 增加
273133 μs，热降频计数未增；端点 SM 时钟为 120/2032 MHz。快照覆盖编译、
验证及计时，不能据此断言每个 trial 的时钟或变化原因。

**保留 N128 / group8 / 均衡 64 clusters / 四级输入 / TMEM 双槽的正式 4096
路径，本轮三个候选归档，不重复运行。** 先前新增 35 项、相关回归 267 项及
共享 A 公共模型回归 29 项，合计 331 项本地检查记录仍有效。本轮只核对新增
原始数据并更新文档，没有修改内核、计时器、阈值，也没有重新运行 GPU 测试。

## 前轮五级复核：Fm3CcJ 交替顺序下 8/8 更快，仍有两个超线样本

[原始八轮样本](results_b300/step10_depth5_balanced.Fm3CcJ/step10_4096/samples.json)、
[汇总](results_b300/step10_depth5_balanced.Fm3CcJ/step10_4096/summary.csv)、
[运行元数据](results_b300/step10_depth5_balanced.Fm3CcJ/step10_4096/run.json) 和
全部编译产物已同步到本地。执行版本为 `79dd8f80ff840a3e15b00436675324cfe3026280`，
Slurm job 27527 / step 0，B300 148 SM / SM103a，TVM 0.26.0 / CUDA 13.0。
内核 SHA256 为 `727a44403b6c1ea0a0499c8212e5796d0fa2b31944929e9ecd50d60be4b961f2`。
七个运行源码哈希与该提交及当前文件均一致；`git_dirty=True`，session 只列出
本次新结果目录未跟踪，没有证据表明计时使用了其他内核代码。

两版本的 4096 数值验证通过，候选六组 K64/128/192/256/320/384 边界各两次，
共 **12 次边界启动通过**。原始顺序严格为 AB/BA 各四次；warmup=10、repeat=30、
seed=0，device/probe/tee 退出码均为 0。以下指标直接从未舍入样本重算：

| 配置 | 中位数 ms | 最大值 ms | 对 baseline 配对加速 | 更快轮数 | 达标样本 |
|---|---:|---:|---:|---:|---:|
| baseline，K64 四级 | 0.139042 | 0.139927 | 1.000000× | — | 5/8 |
| `tmem_input_depth5` | 0.138689 | 0.139322 | **1.003139×** | **8/8** | **6/8** |

五级的配对耗时下降中位数约 **0.313%**，配对节省中位数 **0.435 μs**；
AB/BA 分开统计的配对加速为 **1.002920× / 1.003622×**。两种位置都有正向
信号，支持五级小幅收益，不再仅依赖此前固定 baseline 在前的实验。第 4 轮
差值约 2.741 μs，明显大于典型收益，不能作为常态节省；全部样本均保留。

五级第 1、2 轮仍为 **0.139321597 / 0.139250135 ms**，分别超出原
0.139100 ms 门槛 **0.222 / 0.150 μs**。五级中位数余量仅 0.411 μs／约 0.295%，
汇总 PASS 只表示中位数达标。本次 6/8 不代表总体通过率，也不是全量 pytest
或正式四尺寸验收。**结论为小幅收益得到支持、稳定性目标未完成；五级保留为
后续优化候选，暂不改正式四级内核，不再要求重复这组 4096 对照。**

两个方阵及六个边界共八个构建的 builder/CUDA 指纹均核对通过；其 CUDA、
cubin、NVRTC 参数和版本共 **32 个文件**与旧 `step10_depth5_recheck.sMMqkr`
对应产物逐字节一致。没有 CUDA 后处理差异。4096 两者均 REG=112、STACK/LOCAL=0；
五级只是窄 N 双槽输入深度 4→5，动态 SMEM 从 181248 B 增到 222208 B，
未减少寄存器数；共享 B、group=8、TMEM 双槽和网格不变。

本次与前轮 L2 实验的设备 UUID 都是 `778768b4-6c9e-e483-890e-0812760948ae`，
其 baseline CUDA/cubin/编译参数也一致。前后约 12 s 快照内 SW Power Capping
计数增加 **196969 μs**，热降频计数未增；两张快照的 SM 均为 1095 MHz。
该窗口包含编译、验证和计时，不能将计数增量归给具体 trial，也不能凭端点
快照断言整个计时期间频率固定。小幅优化尚不足以覆盖已观测运行波动。

本轮只更新分析和状态文档，没有修改内核、候选、计时器或阈值，也未运行新的
本地 GPU 测试。已核对原始统计、六份边界记录、源码/编译产物及退出码；
该轮时已知全量记录是下文的 **61 passed / 1 failed**；现已补齐 `9e3b989` 五轮通过证据，见本文顶部。

## 前轮 L2 复核：nqNoLW 三个候选均无收益，保留 group=8

用户回传在 `9ad4a382f6d5e3013aa1440a3d488b678fc35b0c`、B300 148 SM /
SM103a 上执行 `bash run_step10_l2.sh` 的完整终端输出，内核 SHA256 为
`727a44403b6c1ea0a0499c8212e5796d0fa2b31944929e9ecd50d60be4b961f2`。
目录 `results_b300/step10_l2.nqNoLW` 现已同步到本地。本节表格保留首次按
终端六位小数重算的数据；本轮另核对原始样本与 CSV 中位数、平衡顺序和达标数，
结论相同。其 baseline CUDA/cubin/编译参数与最新五级复核一致，GPU UUID 相同。

四个 4096 方阵构建均验算通过，三候选各六组边界各两次，共 **36 次边界启动
通过**。八轮顺序为 ABCD/DCBA/CDAB/BADC，重复两次（A=baseline，B/C/D=
group4/2/1），每个版本位置和两两先后次数均平衡。综合退出码为 0，表示
probe 正常完成；四个中位数均 SLOW，不能写为性能验收通过。

| 版本 | 中位数 ms | 最大值 ms | 对 baseline 配对加速约 | 更快轮数 | 达标样本 |
|---|---:|---:|---:|---:|---:|
| baseline，group8 | 0.139125 | 0.140298 | 1.000000× | — | 3/8 |
| group4 | 0.140091 | 0.140433 | 0.993604× | 0/8 | 0/8 |
| group2 | 0.139975 | 0.140416 | 0.994574× | 0/8 | 0/8 |
| group1 | 0.139717 | 0.140026 | 0.995914× | 2/8 | 2/8 |

打印精度样本（单位 ms，不冒充原 `samples.json`）：

| Trial | group8 | group4 | group2 | group1 |
|---|---:|---:|---:|---:|
| 1 | 0.140298 | 0.140433 | 0.140416 | 0.138814 |
| 2 | 0.139076 | 0.140063 | 0.139777 | 0.138661 |
| 3 | 0.139011 | 0.140371 | 0.140259 | 0.139757 |
| 4 | 0.139113 | 0.140119 | 0.139847 | 0.139582 |
| 5 | 0.139214 | 0.140356 | 0.139932 | 0.139677 |
| 6 | 0.139042 | 0.139793 | 0.140070 | 0.140026 |
| 7 | 0.139137 | 0.139941 | 0.139921 | 0.139956 |
| 8 | 0.139178 | 0.139886 | 0.140018 | 0.139851 |

group4/2/1 的配对耗时增幅中位数约 **0.64%/0.55%/0.41%**；按候选先测、
后测分别统计，三者的配对加速中位数也都小于 1。group1 仅前两轮快，
后六轮都慢，不能凭最小值 0.138661 ms 采用。baseline 中位数超门槛
0.139100 ms 仅 0.025 μs，但有 5/8 样本超线，稳定性问题仍在。
**本轮不采用任何 L2 候选，不重复这组实验，正式内核继续 group=8。**
这否定了本次“缩小分组可改善 4096 性能”的预测，不能推导 L2 对所有配置都无影响。

当时安排用 `bash run_step10_depth5.sh` 复核已有五级输入候选，保持 group=8、
共享 B 和双 TMEM 槽，四级/五级各八轮 AB/BA，warmup=10 / repeat=30 不变。
这是修正旧实验顺序偏置后的复核：`step10_input_ring.9SsKZM` 四版本顺序
不完全平衡，`step10_depth5_recheck.sMMqkr` 双版本始终 baseline 先测；此前
约 0.3%–0.4% 的信号尚不能当作无偏收益。五级此前也有超线样本，未采用。
若新对照没有收益或仍无足够余量，保留失败结果，不靠重复运行挑选 PASS。

本轮没有修改候选实现或正式内核，仅新增复核脚本并记录结果。当前输入环的
源码生成/回退检查及顺序平衡回归 **37 passed，39.30 s**，无跳过；脚本通过
Bash 语法及成功、probe 失败、tee 失败、两者失败、设备失败、非法参数六种模拟。
本机未执行新的 GPU 测量；五级 AB/BA 结果现已回传并核对，见本文顶部。

## 历史全量失败：61 passed / 1 failed，Step 10／4096 超线

用户回传 `uv run python -m pytest tests/ -vs --tb=short` 的完整终端输出：
**62 collected，61 passed，1 failed，85.25 s**。62 = 原有 57 + review 后新增
5 个 Step 10 分支用例；61 是通过数，不是收集总数。五个新增用例全部通过，
唯一失败是原有 `test_multi_consumer[4096]` 的性能断言。其数值验证已通过。

| 项目 | 本次结果 |
|---|---:|
| Step 10／4096 | 0.140561 ms，977.79 TFLOP/s |
| 原评分门槛 | 0.139100 ms |
| 超出门槛 | 0.001461 ms = 1.461 μs，约 1.05% |

命令将日志命名为 `results_b300/pytest_3a9d486_2.log`，但本地尚无该文件，
终端也没有源码指纹；文件名不能证明执行的是哪个 commit 或工作区版本。
这次不能记为全量验收通过，也不能将性能失败归因于新增数值用例。
多步骤耗时均比历史较快验收慢，提示运行条件可能不同，但当前输出不足以
确认时钟、功耗、其他负载或二进制原因。历史达标记录与本次贴线失败均保留；后续 `9e3b989` 五轮通过见本文顶部。

`9ad4a38` 当时准备独立 `tmem_l2_group4/2/1` 候选，与 group=8 baseline 比较。
仅对窄 N 且 TMEM 双槽的路径改变调度分组，保持 512×128 cluster tile、
共享 B、K64 四级、EPI32、双槽同步和 cluster 数不变；单槽与宽 N 回退不变。
4096 的 8×32 个输出任务在 64 clusters 上运行，第一批 group=8 涉及 4096 行 A
和 1024 行 B，group=4 则为 2048 行 A 与 2048 行 B。较小输入覆盖范围可能
改善复用，但不等于实测带宽节省或性能收益。

旧 `k128_step67.TdkZy5/probe/step10_4096_l2_group_4` 已测过 group=4，配对
加速为 0.997264×，略慢。该构建是 512×256 输出 tile、单 TMEM 槽、最多
74 clusters；本轮是当前窄 tile／双槽／64 clusters，不能混用旧实验结论。

该轮脚本为 `bash run_step10_l2.sh`。四版本各八轮，完整两个顺序周期；
每个版本在每个位置各出现两次，每对版本先后各四次。先验证方阵及六组边界，
再用原 warmup=10 / repeat=30 计时；保留全部原始样本、编译产物、UUID、
源码指纹和脚本退出码。SLOW 仍是测量结果，退出码 0 不代表达标。
当时只有候选准备；随后 B300 结果已回传，三候选不采用，见本文顶部。

本地 TVM 0.26 检查：新 L2 候选 **28 passed，48.48 s**，覆盖 SM100a/SM103a
实际生成的初始化与三个角色的调度更新、完整/部分分组、逐输出块唯一覆盖、
TMA/MMA/写回调用不变、双槽 handoff，以及小方阵/单槽/宽 N 的 CUDA 与描述符
一致性。相关 persistent probe、Step 4/5 probe 与 benchmark CLI 回归为
**160 passed，31.67 s**，合计 **188 passed**，无跳过。脚本通过 Bash 语法及
六种隔离模拟：成功、probe 失败、tee 失败、二者同时失败、设备检查失败、非法参数。
这些是本地工具检查，未执行 NVIDIA GPU 数值、计时或 sanitizer。

## Step 9 复核：交换顺序后仍有收益，缓存已移植，后续回归已通过

用户回传了在 `3a9d486a7c4a6718d54ecb261f486487515265d3`、B300 148 SM /
SM103a 上的终端输出，内核 SHA256 为 `1a06fa57…`：4096 结果目录
`results_b300/step9_cache.EWrE9G`，8192 为 `results_b300/step9_cache.BBJEQB`。
**这两个目录现已归档到本地**。本次从各自的 `samples.json` 重算下表，
核对 16 份 builder/CUDA 指纹，目录包含 cubin 和六组边界验证记录。
两次 GPU UUID 均为 `778768b4-6c9e-e483-890e-0812760948ae`。
下方逐轮表保留首次终端打印的六位小数；它不是额外一轮测量，也不证明计时期间时钟恒定。

两次顺序均为 AB/BA/AB/BA/AB/BA/AB（A=baseline，B=cache），两个版本均在
计时前验证通过；每次六组边界各运行两次，共 24 次边界启动通过。两个尺寸的
**28/28 计时样本达标**，两条脚本退出码均为 0。

| 尺寸 | baseline 中位数 ms | cache 中位数 ms | 配对加速约 | cache 更快轮数 | cache 最大值 / 门槛 ms |
|---|---:|---:|---:|---:|---:|
| 4096 | 0.144269 | 0.142462 | **1.012452×** | **7/7** | 0.143025 / 0.150800 |
| 8192 | 0.912429 | 0.895060 | **1.019953×** | **7/7** | 0.895655 / 1.024400 |

终端样本留存如下，单位 ms；这是打印精度数据，不冒充原 `samples.json`：

| Trial / 顺序 | 4096 baseline | 4096 cache | 8192 baseline | 8192 cache |
|---|---:|---:|---:|---:|
| 1 / AB | 0.145334 | 0.142174 | 0.907441 | 0.887966 |
| 2 / BA | 0.144997 | 0.143025 | 0.906532 | 0.888798 |
| 3 / AB | 0.143950 | 0.142462 | 0.906163 | 0.894417 |
| 4 / BA | 0.144317 | 0.142542 | 0.912429 | 0.895060 |
| 5 / AB | 0.144002 | 0.142080 | 0.914103 | 0.895655 |
| 6 / BA | 0.143907 | 0.142182 | 0.913406 | 0.895502 |
| 7 / AB | 0.144269 | 0.142785 | 0.912607 | 0.895279 |

按先后位置分别重算配对比值的中位数，4096 的 AB/BA 为
**1.011987× / 1.012452×**，8192 为 **1.019976× / 1.019953×**。
候选在两种顺序下都更快，这消除了此前“始终第二个测”的具体偏置；仍不等于
GPU 状态恒定或跨 GPU 稳定收益。配对节省中位数约为 1.775 μs / 17.734 μs。
本轮不含 cuBLAS 计时，不能用它更新 cuBLAS 领先结论。

此前已归档的首轮覆盖 1024/2048/4096/8192 与六组边界，小尺寸接近持平。
结合本次大尺寸复核，已将同一基址快照移植到 `hgemm_v9` 的全部合法形状，
不增加仅针对评分尺寸的分支，也不宣称小尺寸有稳定提速。快照位于初始化
CTA/cluster 同步之后，仅替换 TMEM 操作数基址；四级 K64、布局、网格和同步
协议不变。其他九个 builder 的 AST 与 `3a9d486` 一致，Step 10 未修改。
当前内核 SHA256：
`727a44403b6c1ea0a0499c8212e5796d0fa2b31944929e9ecd50d60be4b961f2`。

用户随后执行新增 Step 10 分支测试，终端报告 **5 passed, 6 deselected，
10.22 s**。单/双槽边界、宽 N 短 K、部分 L2 分组五项补充验证已通过；这与
历史全量 57 passed 是分开的运行，不能合并写成“一次全量 62 passed”。

**该轮当时状态是缓存已移植、采用后回归待执行；现已由 `9e3b989` 的五轮
62 项 pytest 与全步骤 benchmark 补齐并通过。** 原准备的
`run_step9_validate.sh`（每尺寸七轮并采集编译产物）保留供重放，不要求为本次
验收重复执行。旧缓存 probe 需在 `3a9d486` 重放，当前代码拒绝重复应用已采用的变换。

本地相关检查 **170 passed，81.86 s**（Step 9 缓存/采用重放、通用 builder、
persistent probe），无跳过。SM100a/SM103a 四方阵与六组边界的 CUDA/TMA
描述符均与已归档候选一致；SM103a 的 CUDA 函数体还直接匹配归档文本。
验收脚本通过 Bash 语法和六种模拟场景：成功、pytest 失败、benchmark 失败、
tee 失败、设备检查失败、非法参数。没有在本机运行 NVIDIA GPU 验证。
...
`3a9d486` 当时只修正 Step 8 注释并补充 Step 10 槽位/phase 说明，Python AST 与
`c788f3a` 相同；当时没有采用 Step 9 缓存或改动配置规则。注释使文件 SHA256
...
重新填充输出；该次尚未执行 GPU 验证，后续用户回传的 5 passed 见本文开头。
原来的 57 项 GPU 验收不包含新增用例，当前全量将收集 62 项。
...
## 历史正式验收：step10_final.vM3h3I，57 项测试与 28 个计时样本全部通过

**该历史版本在记录的 B300 分配下验收通过。** `f165b5e` 收录了在
...
首轮 B300 测量已经完成，结果如下；当时正式内核仍未采用候选。后续 AB/BA
复核和移植状态见本文开头。
...
**五级出现重复的正向信号，但旧顺序偏置仍需修正，稳定达标的目标没有完成。**
