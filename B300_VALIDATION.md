...
## 当前软件优化：窄 N 双 TMEM 槽的输出写回候选（待 B300 测量）

用户给出的 squeue 中仅自身 27689 处于 R，其余均为 PD/Dependency；没有这份
队列支持的其他运行作业竞争证据。按用户要求直接针对当前约 0.140 ms 条件优化
软件，设备策略排查不再作为继续工作的前提。此前运行状态记录保留，不将它
当作停止内核优化的理由。

本次添加 `probe_step10_epilogue.py` 和 `run_step10_epilogue.sh`，通过现有
独立 builder/原计时器机制比较 baseline 与三种写回候选。目的不是减少数学
计算，而是减少输出准备、WG 同步与 TMA store 的串行开销：

| 候选 | 对照 | 改动 | 四级输入下动态 SMEM |
|---|---|---|---:|
| tmem_epi64 | baseline | 每个 consumer 从四次 32 列改为两次 64 列写回 | 197632 B |
| tmem_epi128 | baseline | 一次写回 128 列，用两片 64 列的三维 TMA 描述符 | 230400 B |
| tmem_epi32_double | baseline | 仍四次 32 列，两个 SMEM 槽交替写入与异步读取 | 197632 B |

假设是输出段/最后一个 tile 的尾部同步能节省时间；如果更多 SMEM 或较大
写入批次抵消了节省，候选也可能持平或退化。先前当前路径角色 trace 中 epilogue
约占写回角色合计时间 12.27%，它与其他角色重叠，不能据此预报整体加速 12%。
已有 N64、74 clusters、较小 L2 分组的失败结论继续保留；不重复这些实验。

旧宽 N 输出双缓冲为容纳 SMEM 使用三层输入，不能直接复用它的负面结论推导
当前窄 N 四层输入必然无收益。与此同时，旧窄 N 单 TMEM 槽曾测到 EPI32 优于
EPI64，所以此次也不会把“更少同步”直接等同于更快。三个候选必须同轮比较。
正式两个 consumer 共享 B、输入 ring、MMA、TMEM read/release/phase 和网格均
保持。SMEM 双缓冲使用固定发射 lane，读完成后经 WG barrier 发布复用权限，
每个 tile 最后 drain，再调度下一 tile 或退出。

本地检查两种架构 SM100a/SM103a：四个评分尺寸、六个受影响边界以及单/双槽
和宽 N 的回退。逐项核对生成的 A/B 描述符、MMA/TMEM 协议、输出坐标/覆盖、
SMEM 物理地址、TMA 映射和异步读完前不覆盖的次序；回退 CUDA 与描述符逐字
一致。源码生成检查不能替代 GPU 数值与性能。**尚无本轮候选 B300 结果，未
采用候选到 gemm_kernels.py，也未改变计时器、评分阈值或 GPU tests。**

本地定向测试 **169 passed，134.83 s，零跳过**：新写回候选与 shell 入口、
历史双缓冲 epilogue、persistent 工具以及当前输入 ring 回归。Bash 语法、
Python 编译、CLI help 和 diff 空白检查通过。这不是 B300 cubin 编译或 GPU
性能验收；NVRTC 资源与实际数值还由服务器 probe 在计时前核对。

运行一次 `bash run_step10_epilogue.sh`。四版本八轮平衡计时，全部候选评分
形状与六边界的两次数值验证在计时前执行；保存源码、编译产物、原始样本和
直接对照。结果返回后按同轮收益/最慢余量决定是否采用，并进行原 pytest 与
正式 benchmark 回归。命令和资源表详见 [RUNNING.md](RUNNING.md)。
