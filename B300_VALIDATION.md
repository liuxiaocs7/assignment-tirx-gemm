...
最新全量记录仍是下文的 **61 passed / 1 failed**，Step 9 采用后正式回归仍待完成。
...
## 最新全量反馈：61 passed / 1 failed，Step 10／4096 性能余量待改善
...
确认时钟、功耗、其他负载或二进制原因。历史达标记录保留，最新贴线失败也保留。
...
## Step 9 复核：交换顺序后仍有收益，缓存已移植，正式回归待执行
...
**当前状态是缓存已移植、采用后正式回归待执行。** 下一步运行
`bash run_step9_validate.sh`：全量 62 项 pytest，以及正式 Step 9 四尺寸
benchmark（各七轮），记录原始数据、GPU UUID、编译产物和退出码。二者通过
后才能记录这次采用版本验收通过。旧缓存 probe 需在 `3a9d486` 重放，当前
代码拒绝重复应用已采用的变换。
