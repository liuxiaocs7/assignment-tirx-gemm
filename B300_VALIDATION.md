...
## 最新结果：step10_tmem_sizes.I9nGIJ，按工作量采用实测版本，待正式验收
...
**下一步运行正式 Step 10、七轮 benchmark 及两次全量 pytest**，命令见
[RUNNING.md](RUNNING.md)。本机没有 NVIDIA GPU，不能在这里完成该验收。
最新已完成的正式全量仍是旧生产版本的 **56 passed / 1 failed**；本次 probe
验证与源码重放支持采用，不等于新正式版本已经稳定全过。
