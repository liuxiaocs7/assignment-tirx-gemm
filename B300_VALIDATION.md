...
## 最新 L2 复核：nqNoLW 三个候选均无收益，保留 group=8
...
目录 `results_b300/step10_l2.nqNoLW` 尚未同步到本地；以下按终端六位小数
重算，没有核对该目录的原始 CSV、cubin、GPU UUID 或时钟。
...
下一步用 `bash run_step10_depth5.sh` 复核已有五级输入候选，保持 group=8、
...
本机未执行新的 GPU 测量，五级 AB/BA 结果待 B300 回传。
