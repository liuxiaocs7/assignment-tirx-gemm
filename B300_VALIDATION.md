...
## Review 后修正：测量顺序、结论范围与边界覆盖

对 `c788f3a` 的 review 所指出的两处测量问题均可复现：旧
`trial_order()` 在双版本时始终返回 `[0,1]`，四版本时也没有平衡位置；
`benchmark.py` 则先测完全部 kernel trials，再测全部 cuBLAS trials。
它们影响小幅收益的归因，不改变原始耗时、数值校验或固定作业阈值的 PASS。

现在共用修正后的顺序生成器：双版本按 AB/BA 交替；多版本将每个排列与其
逆序配对，每对之间旋转两个位置。偶数个版本以 `count` 轮为周期，奇数个
以 `2*count` 轮为周期；完整周期内每个版本的位置分布、每对版本的先后次数
均平衡。不完整周期仍可能不平衡，例如七轮双版本为 4 次 AB、3 次 BA。
保留全部样本和顺序，不删除早期或较慢样本。

正式 benchmark 也按 AB/BA 比较 kernel/cuBLAS，计时前分配好两份输出并
验证结果。CSV 新增 `trial_orders`、`cublas_speedup_samples` 与
`paired_speedup_vs_cublas`，有 diagnostics 时同时保存 `timing.json`；
原 `speedup_vs_cublas` 继续表示两个中位数的比值。计时器、warmup/repeat
默认值、误差容限和评分阈值均未改。交替顺序只能减少系统性位置偏差，不能
保证 GPU 状态恒定；小幅优势仍需结合原始样本与运行条件判断。

历史最终验收的 Step 10／4096 kernel 为 0.093980–0.094208 ms，而之后
cuBLAS 为 0.086935–0.106541 ms，中位数比值 1.044894×。这是顺序测量下的
观测值，**不能据此认定稳定领先 cuBLAS 4%**；原 28/28 达标记录保留。
共享 A 201 轮实验也存在位置偏置，约 0.44% 仅作为待复核信号。

正式内核只修正 Step 8 注释并补充 Step 10 槽位/phase 说明，Python AST 与
`c788f3a` 相同；没有采用 Step 9 缓存或改动配置规则。注释使当前文件 SHA256
变为 `1a06fa5715eda5567a5635eb285480ef8c0f2dc67e85da65cf38e6fc57052f85`，
下文 `5515a04…` 仍指历史实测文件。新增五个 Step 10 GPU 边界用例，涵盖
单/双槽切换、宽 N 短 K 和部分 L2 分组，每个构建连续运行两次并用 NaN
重新填充输出；它们尚未在本机执行 GPU 验证。原来的 57 项 GPU 验收不应
改写为新增用例也通过，当前全量将收集 62 项。

TVM 0.26/B300 的验收范围也不等于原课程旧 grader 兼容。主文件保持自包含，
但原 `mlc-ai-tirx-cu130==0.0.1b2` 环境需单独验证，见
[运行环境说明](RUNNING.md#1-当前实现与验证范围)。

本轮使用本地 TVM 0.26，覆盖 **311 个不同工具用例**：相关回归
**230 passed，83.68 s**；当前输入环、共享 A、Step 9 缓存生成检查
**79 passed，84.27 s**；随后给 benchmark CLI 补充 SLOW/UNSCORED 场景，
重跑 **21 passed**（其中 19 项已计入前 230 项）。包括顺序平衡、配对记录、
既有 probe/profile 与 Step 8/10 生成代码重放，无跳过。
修复前新增测试复现了两处顺序缺陷，修复后通过。没有重跑完整工具集；
review 报告的 `c788f3a` 全集结果为 636 passed。未执行 B300 数值、性能或
sanitizer；本地 TVM 检查环境也没有 Torch，直接收集 GPU 套件会因依赖缺失失败。
...
均匹配执行提交与当时的工作区。`dirty=True` 对应版本记录中的新结果目录
...
候选实现时 `gemm_kernels.py` 未改。新增候选保持 Step 9 的四级输入、单 consumer、
...
首轮 B300 测量已经完成，结果如下；当前正式内核仍未采用候选。

`c788f3a` 收录了在 `0c89ba5` 执行的
[step9_cache.1WDeei](results_b300/step9_cache.1WDeei/)：四个尺寸的两版本数值
检查均通过，六组边界在各尺寸进程内各验证两次（共 48 次边界启动），
**56/56 计时样本达标**，probe/tee 退出码全部为 0。

| 方阵尺寸 | baseline 中位数 ms | cache 中位数 ms | 配对加速 | cache 更快轮数 |
|---|---:|---:|---:|---:|
| 1024 | 0.018622 | 0.018610 | 1.002009× | 6/7 |
| 2048 | 0.028867 | 0.028844 | 1.001036× | 5/7 |
| 4096 | 0.144378 | 0.142046 | 1.014611× | 7/7 |
| 8192 | 0.911645 | 0.893722 | 1.020129× | 7/7 |

四份 `samples.json` 的七轮顺序均为 `[0,1]`，baseline 永远先测。
因此 4096/8192 有约 1.5%/2.0% 正向信号，**待交换顺序后确认，暂不采用**；
小尺寸约 0.1%–0.2% 暂按未确认收益处理。下一步仅复核 4096/8192，命令见
[RUNNING.md](RUNNING.md#可选优化step-9-缓存-tmem-基址)。

两版本 NVRTC 选项一致；SASS 中普通静态 `LDS` 指令数为 16→7，但不能将
静态计数直接当作运行时节省。1024/2048 寄存器为 220→186，4096/8192
反而为 **177→194**；所有构建 STACK/LOCAL 为 0，动态 SMEM 仍为
148480 bytes。不能把基址缓存笼统解释成寄存器压力下降。

本轮是 Slurm job 27515 / step 0，物理 UUID 为
`GPU-778768b4-6c9e-e483-890e-0812760948ae`，与最终验收的
`GPU-dadf9f3b-df58-d3fa-07b0-5fe223423db1` 不同；同一主机不等于同一 GPU。
前后快照 SM 时钟均为 1095 MHz，不能据此确定计时期间时钟，也不能仅凭快照
解释所有绝对耗时差异。
...
[样本与顺序](results_b300/step10_share_a_state.fGOjDy/step10/samples.json)、
[时间戳日志](results_b300/step10_share_a_state.fGOjDy/step10.log)、
[GPU 状态](results_b300/step10_share_a_state.fGOjDy/gpu_samples.csv)、
[进程记录](results_b300/step10_share_a_state.fGOjDy/compute_processes.csv)、
[开始快照](results_b300/step10_share_a_state.fGOjDy/gpu_before.txt)、
[结束快照](results_b300/step10_share_a_state.fGOjDy/gpu_after.txt) 和
[作业信息](results_b300/step10_share_a_state.fGOjDy/session.txt)。
与前次七轮状态采集保持同一 Slurm job 27503 / step 0、同一
`GPU-dadf9f3b-df58-d3fa-07b0-5fe223423db1`，warmup=10、repeat=30、seed=0。
四个评分构建、16 组边界各两次及计时后的数值校验全部通过；四版本各 201 个
样本均在门槛内，probe/时间戳/tee 均返回 0，监控错误文件为空。

| 配置 | 中位数 ms | 最大值 ms | 达标样本 | 对 baseline 配对加速 | 对直接对照配对加速 |
|---|---:|---:|---:|---:|---:|
| baseline | 0.112860 | 0.115906 | 201/201 | 1.000000× | 1.000000× |
| 原五级 | 0.112822 | 0.115857 | 201/201 | 1.000715× | 1.000715× |
| 共享 A 五级 | **0.112003** | 0.115504 | 201/201 | **1.004633×** | **1.004371×**（原五级） |
| 共享 A 六级 | 0.112148 | 0.115408 | 201/201 | 1.004471× | 0.999629×（共享 A 五级） |

**保留共享 A 五级作为采用候选，停止推进六级。** 五级比原五级快的样本为
179/201，比正式 baseline 快的为 178/201；直接配对耗时差中位数为
**0.491 μs**，对应约 **0.44%** 配对收益。六级比共享 A 五级快 91/201，
直接配对略慢约 0.04%，未显示值得增加一级缓冲的收益。原五级相对 baseline
仅约 0.07%。共享方式变化是当时的候选解释；review 后发现位置偏置，归因待复核。

为判断收益是否只来自最初快段，将全部 201 轮按索引分成四个连续块；这些是
诊断切片，不是新评分，也不删掉任何样本。五级对原五级在每段均略快：
...
按实际四种测量顺序分别统计，直接配对加速仍为 1.003726–1.005159×。
这比七轮数据更支持持续的小收益，但 201 轮来自同一进程，不等于 201 次独立
验收，也不能由这些计数推导跨 GPU 的通过概率。四种旧顺序仍让 baseline
只处于第 1/3 位、原五级只处于第 2/4 位，按旧顺序分组不能消除这种偏差。

### 长窗口确认了什么

最后边界验证到最后计时日志为 **06:26:57.337–06:27:01.428 UTC，共 4.091 s**，
计时窗口内有 20 个 GPU 状态采样点。计时前为 2032 MHz，第一个窗口内采样为
1605 MHz，后续 19 点为 **1387–1447 MHz**；GPU 温度为 45–55℃。
功耗读数逐步上升，在窗口后段约 **1050–1067 W**，设置上限 1100 W，
显存时钟维持 3996 MHz。GPU 没有保持单一频率，但连续负载后的工作范围
已经得到多点覆盖。

SW Power Capping 累计值从 3874322877 增至 3878518646 μs，增加
**4195769 μs（4.195769 s）**；SW/HW Thermal Slowdown、Sync Boost 和
HW Power Braking 计数仍未增加。这支持持续负载下的功耗限制，未显示热降频。
计数覆盖整次运行，不能与 4.091 s 评分窗口相除后宣称精确占比。
26 条进程记录只见本项目 Python PID 2630000，其中四条落在计时窗口，
没有观察到其他可见计算进程；稀疏采样仍不能完全排除短时竞争。

当前 baseline 自身最慢样本相对 0.139100 ms 门槛仍有 **16.67%** 余量，
共享 A 五级最慢余量为 16.96%。这说明当前 GPU/运行条件下，正式实现也能
在本轮持续达标；这份大余量不能归功于只有约 0.46% 的候选相对收益。
历史角色诊断用过另一 UUID/时钟状态，历史贴线失败仍保留，不能被本轮覆盖。

七个源码指纹匹配 `01a2248`；20 组构建各五份文件（builder、CUDA、cubin、
...
## 前次状态采集：step10_share_a_state.X3xsC4，抓到降频与功耗限制计数增长

`4d70540` 补齐在 `74cb1e0` 执行的状态采集：
[summary](results_b300/step10_share_a_state.X3xsC4/step10/summary.csv)、
[原始样本](results_b300/step10_share_a_state.X3xsC4/step10/samples.json)、
[时间戳日志](results_b300/step10_share_a_state.X3xsC4/step10.log)、
[GPU 状态](results_b300/step10_share_a_state.X3xsC4/gpu_samples.csv)、
[进程记录](results_b300/step10_share_a_state.X3xsC4/compute_processes.csv)、
[开始快照](results_b300/step10_share_a_state.X3xsC4/gpu_before.txt)、
[结束快照](results_b300/step10_share_a_state.X3xsC4/gpu_after.txt) 和
[作业信息](results_b300/step10_share_a_state.X3xsC4/session.txt)。
数值校验全部通过，16 组边界各验证两次，28 个计时样本全部达标。
probe、时间戳、tee 退出码均为 0；两项监控错误文件为空。

| 配置 | 中位数 ms | 最大值 ms | 达标样本 | 直接对照 | 直接配对加速 |
|---|---:|---:|---:|---|---:|
| baseline | 0.115294 | 0.117861 | 7/7 | baseline | 1.000000× |
| 原五级 | 0.115084 | 0.116100 | 7/7 | baseline | 1.000046× |
| 共享 A 五级 | 0.109014 | 0.115197 | 7/7 | 原五级 | 1.005498× |
| 共享 A 六级 | 0.114940 | 0.118131 | 7/7 | 共享 A 五级 | 0.997550× |

共享 A 五级相对原五级约 0.55% 配对收益，5/7 轮数值上更快；六级约慢 0.25%，
只有 1/7 更快。五级中位数 0.109014 ms 恰是第 4 轮转换区间的样本，不能当作
稳定执行速度。第 5–7 轮仅用于说明时序变化：四版本耗时均约 0.115–0.118 ms；
不删除前面样本，不用尾段另造评分。正式内核继续保留。

### 同机与同卡、降频与热降频的证据

本次主机 `dev-slurm`、Slurm job 27503 / step 0，分配记录 `step_gpus=2`，
CUDA 逻辑设备为 0。CUDA 与 NVSMI UUID 均对应
`GPU-dadf9f3b-df58-d3fa-07b0-5fe223423db1`，驱动 580.126.09。
此前 `step10_current_profile.3cjntP` 的
[GPU 快照](results_b300/step10_current_profile.3cjntP/roles/step10_4096/gpu_trial01_before.csv)
对应 **`GPU-3924bd78-8b2f-8e85-3f20-8b0687d0bb0a`**，14 份轮前后快照
均为 1095 MHz；本次编译/验证间隔内多为 2032 MHz。
**主机可以相同，但这两次用的是不同物理 GPU。** 上次共享 A 和原五级独立
复测没有 UUID 记录，不能推断它们一定使用哪张卡，也不能用快照频率直接
换算不同运行的 kernel 耗时。

本次计时日志从 06:14:13.928 至 06:14:14.055 UTC，相差 127 ms；最后一次
边界校验到最后计时日志约 131 ms。GPU 状态采样间隔 200 ms，窗口内只有
**一个**状态点：

| 状态采样时间 UTC | SM MHz | 显存 MHz | GPU 温度 | power.draw W |
|---|---:|---:|---:|---:|
| 06:14:13.816，计时前 | 2032 | 3996 | 35℃ | 247.95 |
| 06:14:14.016，慢段内 | **1507** | 3996 | 46℃ | 302.39 |
| 06:14:14.217，计时后 | 2017 | 3996 | 37℃ | 374.48 |

前后快照的 **SW Power Capping 从 3869904012 增至 3870135699 μs，增加
231687 μs（231.687 ms）**。SW/HW Thermal Slowdown、Sync Boost、HW Power
Braking 计数均未增加。P0 始终保持，说明 P0 不能当作时钟恒定的证据。
功耗限制为 1100 W；CSV 的低功耗读数不是逐 kernel 峰值，不能用它否定
累计限制事件。前后快照显示 Not Active 也不表示中间未发生限制。

这些记录支持“本轮发生功耗限制，且慢段观察到 SM 降频”。累计计数覆盖整个
进程运行窗口，时长甚至长于评分日志窗口，不能全部归到这 28 个样本；也不足以
断言每个样本的时钟或历史贴线失败的全部根因。温度计数未增，没有本轮热降频
证据。22 条计算进程记录只看到 PID 2606727、路径为本项目 Python；1 s 采样在
计时窗口内没有点，只能说未观察到其他可见计算进程，不能完全排除短时竞争。

七个源码指纹匹配 `74cb1e0`；20 个评分/边界构建各自的 builder、CUDA、cubin、
NVRTC 参数、版本文件与前次共享 A 实验共 **100 份文件逐字节一致**。
原始样本重算 summary 一致。性能变化不是本轮重新生成了其他候选代码。

### 下一步：延长观测，保持每轮测量定义

优先检验连续负载下的功耗/时钟状态：如果限制导致快慢区间转换，延长相同
四版本序列应让转换和后续状态获得更多采样覆盖。保留同一 Slurm 分配，使用
`bash run_step10_state.sh --trials 201`；预计计时阶段从约 0.13 s 延长至数秒，
具体长度取决于实际执行。仍为每轮 warmup=10、repeat=30，全部数值验证和样本
保留，不主动锁频、提高功耗或丢弃转换期。此命令用于诊断，不代替正式 pytest。

如果粗粒度时钟记录仍不能解释变化，再检查宿主发射、缓存或调度因素。
共享 A 六级尚无额外收益，暂不采用；五级也须在可比较状态下复测后决定。
不需要再执行原七轮状态采集，避免重复无法覆盖足够状态点的短窗口。
包装脚本增加可选 `--trials`，缺省仍为 7；probe、正式内核及评分规则未修改。
新增入口通过 Bash 语法检查及 14 种本地模拟（默认/201 轮、故障、参数校验）；
本轮未重跑 GPU 或完整工具回归，已有完整回归仍为 **614 passed**。

## 前次结果：step10_share_a.r7r3iv，全部达标，但运行内漂移大于候选收益

`64b52d4` 补齐了在 `f029ed7` 执行的共享 A 实验：
[summary.csv](results_b300/step10_share_a.r7r3iv/step10/summary.csv)、
[原始样本与顺序](results_b300/step10_share_a.r7r3iv/step10/samples.json)、
[运行元数据](results_b300/step10_share_a.r7r3iv/step10/run.json)、
[日志](results_b300/step10_share_a.r7r3iv/step10.log) 和
[退出码](results_b300/step10_share_a.r7r3iv/probe_exitcode.txt)。
四个 4096 评分构建通过数值校验；原五级六个边界、共享 A 两版本各五个边界
共 16 组、32 次 untimed launch 全部通过，计时后的输出校验也通过。
沿用 warmup=10、repeat=30、trials=7、seed=0，28 个计时样本均低于门槛。

| 配置 | 中位数 ms | 最大值 ms | 达标样本 | 直接对照 | 直接配对加速 |
|---|---:|---:|---:|---|---:|
| baseline | 0.111850 | 0.115052 | 7/7 | baseline | 1.000000× |
| `tmem_input_depth5` | 0.111706 | 0.115165 | 7/7 | baseline | 1.000540× |
| `tmem_share_a_depth5` | 0.110606 | 0.115228 | 7/7 | tmem_input_depth5 | 1.002075× |
| `tmem_share_a_depth6` | 0.111484 | 0.115010 | 7/7 | tmem_share_a_depth5 | 0.999692× |

**本轮达标，尚不能据此认定共享 A 修复了稳定性。** baseline 与原五级的
builder、CUDA、cubin、NVRTC 参数及版本文件都与上次复测逐字节一致；
但 baseline 中位数从 0.139428 降至 0.111850 ms，耗时减少约 **19.8%**。
本轮 baseline 最小/最大为 0.094261/0.115052 ms，相差约 **22.1%**。
这种整体变化远大于候选之间的差异，不能归因于新布局。

时间顺序也显示同一轮内发生了变化。第 3 轮依次测得：
共享 A 五级 **0.091207** → 共享 A 六级 **0.093982** → baseline **0.110364**
→ 原五级 **0.110489 ms**。前后两组已不在同一耗时区间；同轮比值仍可能
混入时序漂移。共享 A 五级对 baseline 的汇总为 1.011254×，但相对原五级
的直接配对收益只有约 **0.21%**，5/7 轮数值上更快；六级相对共享 A 五级
约为 **0.999692×**，3/7 更快，没有明确额外收益。均不足以作采用依据。

七个运行源码指纹匹配 `f029ed7`；20 个评分/边界构建的 builder/CUDA 哈希
匹配各自记录，没有额外 CUDA 文本变换。四个评分 cubin 各不相同，资源报告
均为 REG=112、STACK=0、LOCAL=0；共享 A 五级/六级确实是独立编译的候选。
由原始样本和原评分常量重建的四行 summary 与保存值一致。

现有元数据只有型号、SM 数、CUDA 可见序号等，缺少物理 GPU UUID、驱动版本、
作业信息和本轮时钟/功耗/温度记录。不能断言换卡、热降频、功耗限制或竞争
任务中的某一项是根因；新增验证和四版本测量也改变了运行上下文。
全部样本保留为有效观测，不删除前几轮，也不选择最快区间作为新成绩。

下一步先用同一实验采集 GPU 身份和运行状态，命令见
[RUNNING.md](RUNNING.md#共享-a-结果漂移时的状态采集复测)。采样只提供粗粒度
相关线索，不是逐 kernel 时钟证明。保持正式内核与评分规则，暂不采用共享 A
五级或六级，也不继续增加候选；先判断整体变化，再在可比较条件下评估收益。
本轮只更新文档并核对已保存产物；完整本地工具回归仍为此前的 **614 passed**。

### 后续采集入口

用户说明各轮应在同一台机器上执行，因此先按同机处理；同机不排除运行状态
变化，也不能由耗时直接断言降频。新增 [run_step10_state.sh](run_step10_state.sh)
包装原四版本 probe，记录 GPU UUID、Slurm 信息、200 ms 状态样本、1 s 可见
计算进程样本和带毫秒级接收时间戳的日志。probe、正式内核、warmup/repeat
及评分规则不变；不锁频、不调整功耗。结果仍需回传后分析。

脚本通过 Bash 语法检查，六种本地模拟覆盖正常结束、probe/tee/时间戳失败、
监控不可用与 CUDA 初始化检查失败；退出码和监控清理均符合预期。这是新增
包装脚本的检查，未重跑完整工具集或 GPU 测试。

## 共享 A 实验设计：两个 consumer 沿 N 排列

五级输入环的独立复测仍有 3/7 超线（实测见后文）。本实验优先改变输入复用
方式，在相同计算量下减少 TMA 请求字节，再比较节省空间后的更深预取。
这两项已实现为独立 probe，**首轮 B300 结果见上节，尚未采用到正式内核**。
...
L2 分组。边界各两次，全部通过后才计时。首轮数值验证通过，性能结论受运行
漂移限制，详见上节。
...
## 前次复测：step10_depth5_recheck.sMMqkr，五级收益重复出现，但 3/7 超线
