...
`9e3b989..32d4fa8` 的正式内核、计时评分代码与 GPU tests 一致。新目录目前未在
本机归档，尚未读取它们的 `version.txt`，也没有本次 GPU UUID、频率或功耗证据。
其余两个目录仅显示存在，不能推断它们的测试结论。
...
本机无 NVIDIA GPU，尚未执行这个新的 GPU 诊断流程，不宣称性能已修复。
