...
## 最新正式验收：d283549，用户反馈五轮全过

记录日期：2026-09-14。正式内核提交
`d2835492913d94532bcd160ea79891a8f0e9d836`，B300 SXM6 AC / 148 SM /
`sm_103a` / TVM 0.26.0 / PyTorch 2.14.0+cu130 / CUDA 13.0 / NVRTC，
ptxas `register-usage-level=10`。用户提供的结果目录名为
`results_b300/step10_tmem_adopt.OQ0WHS`。

**采用后的正式版本已经通过 GPU 验收。** 用户随后反馈累计五轮全量测试均通过；
当前对话展示了其中两轮完整的 `57 passed` 日志。以下数值转录自用户贴出的
正式输出，该目录未保存在当前本地工作区，尚未逐文件核对其 CSV、原始样本和
二进制。其余三轮没有完整日志，因此不补写运行时间或逐项成绩。

| 验证入口 | 已展示结果 | 用时 / 范围 |
|---|---|---|
| `pytest tests/test_step10.py` | **6 passed** | 13.19 s；四个评分形状与矩形 K64/320 |
| `benchmark.py --steps 10 --trials 7` | **四个尺寸中位数均 PASS** | warmup=10，repeat=30，seed=0 |
| 全量 pytest 第 1 轮 | **57 passed** | 76.33 s |
| 全量 pytest 第 2 轮 | **57 passed** | 76.43 s |
| 用户累计复测反馈 | **五轮全过** | 包含上述两轮；其余逐轮明细未展示 |

### 正式 Step 10 成绩

| 尺寸 | 当前配置 | 单独 pytest ms | 全量第 1 轮 ms | 全量第 2 轮 ms | benchmark 中位数 ms | 门槛 ms |
|---|---|---:|---:|---:|---:|---:|
| 1024 | N128 / EPI32 / 单槽 | 0.018695 | 0.018734 | 0.018687 | 0.018547 | 0.032500 |
| 2048 | N128 / EPI32 / 单槽 | 0.027318 | 0.027557 | 0.027907 | 0.027124 | 0.045500 |
| 4096 | N128 / EPI32 / 双槽 | **0.138047** | **0.137666** | **0.137785** | **0.137610** | **0.139100** |
| 8192 | N256 / EPI64 / 单槽 | 0.869177 | 0.868961 | 0.866174 | 0.868908 | 0.946400 |

4096 的三次已展示 pytest 成绩均低于门槛，最慢为 0.138047 ms，余量约
**1.053 微秒 / 0.757%**；benchmark 中位数余量约 **1.490 微秒 / 1.071%**。
此前 B-first 版本约 0.13928 ms 的性能失败不能再当作当前版本状态。重复通过
支持保留 `d283549` 为已验收基线，进一步优化的重点是扩大余量，而非修复一个
当前已复现的失败。尚不能据此保证不同 GPU 状态、软件版本或任意形状都达标。

benchmark 汇总给出 115.78 / 633.38 / 998.76 / 1265.40 TFLOP/s，
对应 cuBLAS 中位数 0.006913 / 0.021270 / 0.127507 / 0.851449 ms。
七轮原始样本未在正式输出中展开，因此这里不声称全部 28 个样本达标，
也不把汇总中位数作为最大值。正式评分仍使用原 pytest 的单轮批量平均计时。

benchmark 输出 `dirty=True`，但其中的 `gemm_sha256` 为
`5515a04dfc3018bff2fe06e7f1f00681db4ee4ce8b376f4098f2348d85989b6c`，
与 `d283549` 及当前内核一致。贴出的摘要不足以核对其他所有文件的状态。
第二轮全量中出现 Slurm time-limit 消息，随后仍有完整的 `57 passed` 摘要；
可确认展示的 pytest 结果通过，Slurm 作业最终退出状态没有单独提供。

本机此前的完整工具/源码生成检查为 **557 passed，299.28 s**；本次只更新文档，
保留内核、测试、数值容差、评分门槛和计时器。各步优化原理、采用证据及后续
优先级见 [OPTIMIZATION_GUIDE.md](OPTIMIZATION_GUIDE.md)。
[RUNNING.md](RUNNING.md) 的命令用于复现或未来代码改动后的验证，当前无需因
文档更新重跑 GPU。

以下保留各实验当时的结论；历史段落中的“下一步”“待验收”描述的是当时状态，
当前状态以上述正式验收为准。
