...
Explicit tmem_split_ready separates B/A0/A1 completion notifications while
retaining the shared input-free barrier and all data buffers on that path.
Explicit tmem_k64_sw64 / tmem_k32_depth* compare input layout, granularity
at equal capacity, then depth, retaining two consumers that share B.
...
from probe_step10_ready import READY_VARIANT, READY_VERIFY_SHAPES, ready_builder_source
from probe_step10_granularity import (GRANULARITY_CONFIGS, GRANULARITY_CONTROLS,
                                      GRANULARITY_VERIFY_SHAPES, granularity_builder_source)
...
STEP_VARIANTS = {
    ...
    10: ("baseline", "mma_wait_64ns", "tmem_load_16", "tmem_load_64",
         ...
         *EPILOGUE_CONFIGS, READY_VARIANT, *GRANULARITY_CONFIGS),
}
...
EXPERIMENT_CONTROLS = {**{v: "cache_tmem_base" for v in CACHE_EXPERIMENTS},
                       ...
                       **GEOMETRY_CONTROLS, **EPILOGUE_CONTROLS, READY_VARIANT: "baseline",
...
VERIFICATION_SHAPES = {
    ...
    **{variant: EPILOGUE_VERIFY_SHAPES for variant in EPILOGUE_CONFIGS},
    READY_VARIANT: READY_VERIFY_SHAPES,
    **{variant: GRANULARITY_VERIFY_SHAPES for variant in GRANULARITY_CONFIGS},
}
...
def variant_builder_source(source, step, variant):
    ...
    if step == 10 and variant == READY_VARIANT:
        return ready_builder_source(source)
    if step == 10 and variant in GRANULARITY_CONFIGS:
        return granularity_builder_source(source, variant)
...
def main(argv=None):
    ...
    metadata = dict(run_metadata(), target=str(target), gpu=device.name,
                    ...
                    probe_step10_ready_sha256=hashlib.sha256(Path(__file__).with_name("probe_step10_ready.py").read_bytes()).hexdigest(),
                    probe_step10_granularity_sha256=hashlib.sha256(Path(__file__).with_name("probe_step10_granularity.py").read_bytes()).hexdigest(),
