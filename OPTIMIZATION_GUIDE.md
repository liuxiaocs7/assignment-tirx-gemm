...
下一项尚未测量的候选是**把四份 K64 输入拆成八份 K32 输入，保持总缓冲容量相同**。
可以把它理解为相同货物换成小箱分批送到计算组：可能更早开算，也可能因为请求和
同步次数翻倍而变慢。实验先单独比较所需的 64 字节共享内存布局，再比较等容量
粒度，最后比较八级/十级容量，保留两个共享 B 的 consumer。所有候选仍与正式版
同轮对照，只有明显收益且全部观测样本达标，才进入复测和采用；见
[运行说明](RUNNING.md)。这里描述的是实验设计，不是已实现的性能提升。
...
| [paired_experiments.csv](docs/optimization_data/paired_experiments.csv) | 40 组显式直接对照、逐对收益、更快轮数、测量位置和采用状态 |
