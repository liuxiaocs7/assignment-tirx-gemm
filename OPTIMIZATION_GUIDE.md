...
用户目前只能用一张 GPU，后续 `step10_runtime._v5qw220` 已做同卡对照：把 30 次计算打包成 CUDA Graph 后，配对耗时下降约 2.2%–2.5%，但普通提交在八秒负载前后都约 0.140 ms，仍只有 2/16 样本达标。负载期间 80 次采样的 SM 时钟全为 1095 MHz，大部分利用率为 99%–100%，功耗限制累计时间增加 8.161490 秒、热限制未增加。这些状态可供设备策略核对，但用户要求直接推进软件优化，保留四级输入的 64/128 列写回与 32 列 SMEM 双缓冲已完成八轮平衡测量：三个候选全是 0/8 更快，不采用。后续拆分 B/A0/A1 就绪通知的实验也已完成：5/8 更快、配对耗时只下降 0.049%，候选仍有两个样本超线，未采用。不能直接认定人为锁频，也不能用 Graph 数据替代原评分。详细原始记录和解释见 [验证记录](B300_VALIDATION.md)。

用户提供的增量 review 与原始记录一致：上一轮“验收未完成”的 P1 应关闭。Step 9 两份复核目录已经归档；几何候选已经测过、没有采用。本文及运行说明更正了这些状态遗漏。

最新的输入粒度实验也已完成：**K64/四级仅改成 SW64 输入布局，耗时就增加约28.155%；
K32/八级没有补回损失，十级还略慢**。三个候选全部0/8快于正式版，均未采用。
四版寄存器都是112且无溢出，这轮不能解释成“缓冲变深导致寄存器溢出”，也不能
把全部28%的退化归因于K32请求翻倍。正式版仍有2/8样本超线；详见下文
[输入粒度分析](#8-把大箱拆成小箱布局成本可能先抵消收益)及[验证记录](B300_VALIDATION.md)。
...
这点也得到新 [写回实验](results_b300/step10_epilogue.xVWC2d/step10_4096/samples.json)的支持：

| 4096 配置 | 中位数 ms | 同轮配对耗时增加 | 更快轮数 | 达标样本 | REG |
|---|---:|---:|---:|---:|---:|
| 原版：四次 32 列写回 | 0.139205 | — | — | 2/8 | 112 |
| 两次 64 列写回 | 0.141452 | 1.614% | 0/8 | 0/8 | 105 |
| 一次 128 列写回 | 0.143390 | 2.978% | 0/8 | 0/8 | 100 |
| 32 列双 SMEM 缓冲 | 0.140787 | 1.120% | 0/8 | 0/8 | 128 |

八轮中四个版本在每个测量位置各出现两次。三个候选的数值及边界验证通过，
但每一轮都慢于原版，全部不采用。加宽版本确实把二进制写回指令位置从四处减成
两处/一处，寄存器也更少且没有溢出，结果仍然更慢。

给初学者的解释是：流水线里负责打包出货的人经常等前面生产完成，给他换一个
更大的箱子，不一定能让整条生产线更快。当前角色 trace 里，写回角色约 86.19%
时间在等 MMA；剩余写回工作还会和下一批计算重叠。因此减少打包步骤，未必
缩短总完成时间。同时大批次搬运、缓冲布局和指令调度也变了，需要实际测量。
本轮没有逐候选硬件 profile，不能断言究竟哪一项成本增加了多少。

### 7. 等自己需要的输入，逻辑更细不代表实测更快

[拆分输入通知实验](results_b300/step10_ready.FAnkEQ/step10_4096/samples.json)
检验的是：两组各等共享 B 和自己的 A，能否避免共同等待另一组 A 的时间。
输入总字节和数学运算没有减少，输入缓冲仍等两组计算都完成才覆盖。

| 4096 配置 | 中位数 ms | 最大值 ms | 更快轮数 | 达标样本 |
|---|---:|---:|---:|---:|
| 共用输入就绪通知 | 0.139018 | 0.139427 | — | 5/8 |
| B、A0、A1 分开通知 | 0.138973 | 0.139199 | 5/8 | 6/8 |

八轮 AB/BA 各四轮，数值和八组边界验证均通过。逐对耗时下降中位数只有
**0.049167% / 0.068267 μs**，5 轮快、3 轮慢；精确加速比 **1.000491993×**
被终端三位小数显示成 1.000。候选第 2、8 轮依然超线，其中位数与门槛之间
仅有约 **0.127 μs**，不能将两行 PASS 当作稳定达标。因此不采用这个候选。

好比两个厨师共用一份调料，各自需要一份配菜：分别通知“你的材料到了”可能
减少等待，但材料原本就差不多同时到，或发通知本身增加工作，就可能没有净收益。
这里是可能的机制，尚未测出各项耗时。两版 REG 都为112、STACK/LOCAL 都为0，
不能用寄存器节省来解释。编译器还把 MMA 自动展开从四阶段改成两阶段，静态
指令数减少，但循环迭代更多、总乘加量相同，也不能据此说计算量减半。

这轮说明当前具体改法基本持平，并不证明所有输入调度优化都无效。后续应争取
数微秒的原计时余量；不靠这点微小差异改变正式配置，也不把 6/8 当作未来通过率。

### 8. 把大箱拆成小箱，布局成本可能先抵消收益

[输入粒度实验](results_b300/step10_granularity.nVKp6r/step10_4096/samples.json)
让原来的四份K64输入变成八份K32输入，总预取字节数相同，希望计算组更早收到
第一份数据。因为K32配套使用SW64输入布局，特意插入一个仍为K64/四级、仅改
布局的版本，区分两种变化。

| 配置 | 中位数 ms | 对正式版配对耗时增加 | 快于直接对照 | 达标样本 |
|---|---:|---:|---:|---:|
| 正式K64/四级/SW128 | 0.138956 | — | — | 6/8 |
| K64/四级/SW64 | 0.178131 | 28.155% | 0/8 | 0/8 |
| K32/八级/SW64 | 0.178197 | 28.264% | 3/8（对K64/SW64） | 0/8 |
| K32/十级/SW64 | 0.179044 | 28.716% | 1/8（对K32/八级） | 0/8 |

**还没拆小箱，仅改变货架上的摆放方式，就已经明显变慢。** swizzle是共享内存
地址的排列规则；128B/64B不是矩阵精度，也不是读入数据总量。这次布局变化
同时改变TMA张量描述符与MMA取数地址。之后在SW64下拆小箱基本持平，再多放
两箱没有改善。所以三个候选都不采用，也不继续试更多缓冲级数。

四版REG都是112、STACK/LOCAL均为0；前三版共享内存都是181248字节，十级才
增加到222208字节。不能把第一项的28%退化归因于资源用量增加或寄存器溢出。
合理的后续研究对象是布局对应的取数路径，但这轮没有候选的硬件计数器，不能
直接断言发生了多少bank conflict或Tensor Core取数吞吐下降。

K32把每份的乘加量减半，但份数翻倍，总计算量相同；TMA请求和同步次数则增加。
这个额外成本在本次SW64对照上没有表现成28%的独立损失：K32/八级相对K64/SW64
的配对耗时只增加约0.108%。这种对照链让我们能拒绝无效配置，同时避免讲错原因。
...
| [paired_experiments.csv](docs/optimization_data/paired_experiments.csv) | 45 组显式直接对照、逐对收益、更快轮数、测量位置和采用状态 |
