...
@pytest.mark.parametrize("shape", [(size,) * 3 for size in (1024, 2048, 4096, 8192)]
                                 + list(BOUNDARIES))
def test_cache_hoists_base_after_cluster_sync_and_preserves_protocol(arch, shape, tmp_path, monkeypatch):
...
def test_step9_cache_refuses_reapplication_and_unpublished_allocation():
