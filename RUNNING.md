...
**最新全量反馈为 61 passed / 1 failed（共 62 项），Step 10／4096 性能超线。**
0.140561 ms 比原门槛 0.139100 ms 高约 1.05%；数值验证及五个新增边界用例
通过。L2 分组复核已经完成，三个候选均更慢而不采用；五级输入八轮 AB/BA
复核也已完成，8/8 更快，但仅 6/8 达标、最慢 0.139322 ms。小幅收益得到
支持，稳定性目标仍未完成；正式内核暂保留四级，无需重复这两组 probe。
终端日志名 `pytest_3a9d486_2.log` 不能代替源码指纹，不能据此确认执行版本。
Step 9 缓存采用后正式回归仍待完成。历史结果
...
已移植同一缓存改动，采用后仍需全量 pytest 与正式 Step 9 benchmark 通过。
...
当前全量为 62 项；最新一次是 61 passed / 1 failed，历史 57 项验收和五项单独
通过不能合并成新版本全量通过。以下命令仅供重放：
...
### Step 10 网格与 N64 tile 对照：待 B300 实测

同步本轮代码后，在当前 B300 分配内运行：
...
看 `vs_control`、候选先测/后测时的收益、最大耗时和逐轮达标数。
优先寻找约 **≤0.135 ms** 且本轮没有超线样本的候选；这是优化筛选目标，
原评分门槛仍是 **0.139100 ms**。即使八轮全部达标，也不能替代后续正式
四尺寸 benchmark 与全量 pytest。退出码 0 表示 probe 正常完成，SLOW 仍须记录。
