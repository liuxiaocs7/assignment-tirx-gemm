...
通过。L2 分组复核已经完成，三个候选均更慢而不采用；下一步运行下文
`bash run_step10_depth5.sh`，修正旧顺序偏置后复核已有五级输入候选。
...
两次均使用修复前的顺序生成器，双版本复测始终 baseline 先测，因此小幅
正向信号尚需排除顺序偏置。L2 三候选均更慢后，下一步只复核四级/五级，
保持 L2 group=8、共享 B、K64 和双 TMEM 槽：
...
五级六组短 K 边界，每组两次。只有实测后才能决定是否采用；此前幅度很小，
即使中位数通过也不能忽略超线样本。无需重新跑 L2 或 201 轮共享 A 实验。

以下旧命令仅供历史重放，当前复核使用上面的八轮脚本：
