...
> 最新单卡诊断（2026-09-15）：`step10_runtime._v5qw220` 已归档。Graph 在
> 16/16 对中更快约 2.2%–2.5%，普通提交仍仅 2/16 样本达标。持续负载前后
> 普通中位数约 0.140 ms；负载遥测 80/80 次采样 SM 为 1095 MHz，功耗限制
> 累计时间增加 8.161490 s、热限制未增加。用户要求继续软件优化，当前推进
> 下述写回候选实验，不以管理员排查为前提，也不必原样重复运行状态诊断。

### 当前要运行：Step 10 窄 N 双槽写回优化

在当前 B300 分配、仓库根目录执行：

```bash
bash run_step10_epilogue.sh
```

这次改的是内核写回代码：原来每个 consumer 的 128 列输出分四次 32 列 TMA
store，每次等待读取 SMEM 完成后才复用缓冲区。候选减少 store/同步次数，或用
两个 SMEM 缓冲交替工作。**尚无候选 B300 性能数据，不能预先承诺提速。**

| 版本 | 每个 consumer 每 tile 的 TMA store | 输出 SMEM 缓冲 | 4096 动态 SMEM |
|---|---:|---|---:|
| baseline | 4 次 × 32 列 | 单缓冲 | 181248 B |
| tmem_epi64 | 2 次 × 64 列 | 单缓冲 | 197632 B |
| tmem_epi128 | 1 次 × 128 列 | 单缓冲 | 230400 B |
| tmem_epi32_double | 4 次 × 32 列 | 双缓冲，允许前一块读 SMEM 与后一块准备重叠 | 197632 B |

三个候选均以 baseline 为直接对照，保留 K64 四级输入、共享 B、N128、TMEM
双槽、group8 和均衡 cluster 网格。只作用于原规则选出的窄 N persistent 双槽
分支；单槽和宽 N 回退保持原 CUDA 与宿主 TMA 描述符。EPI128 使用两片 64 列
的三维 TMA 映射。双 SMEM 缓冲由固定 lane 发出所有 store/commit/wait，
wait(1) 后经 WG 同步才允许复用旧缓冲，tile 末尾 wait(0) 排空。

这不同于旧宽 N epilogue_double_buffer：旧版本为腾出空间将输入深度降为 3，
当前窄 N 方案可保持深度 4。以前在窄 N 单 TMEM 槽上 EPI32 优于 EPI64，
也不能预先推导当前双 TMEM 槽上扩大写回会获益，因此本次独立比较全部候选。

脚本先验证四个 4096 构建；每个候选验证六个短 K/跨 tile/L2 尾组边界，各跑
两次。随后用原 CUDA-event 计时器、warmup=10/repeat=30、seed=0 测八轮。
四个版本各位置出现两次，每对先后各四次；全部数值验证通过后才开始计时。
没有 CUDA Graph、改门槛、换卡或锁频动作。

结果保存到 `results_b300/step10_epilogue.*`，包含变换前后 builder、CUDA/cubin、
资源报告、原始顺序/样本、summary 和退出码。综合退出码 0 只表示实验完整，
SLOW 仍保留；采用须看相对 baseline 收益、最慢样本余量，并通过正式全量回归。
目标是原计时下约 ≤0.135 ms 的余量，最终门槛仍为 0.139100 ms。
本次本地工具与源码生成检查 **169 passed，零跳过**；尚未执行本轮 B300
候选测试。请将新增脚本与 probe 改动一并同步后运行。
