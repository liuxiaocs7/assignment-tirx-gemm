...
> 最新聚焦诊断（2026-09-15）：`step10_diagnose.MBawzh` 已归档，GPU UUID
> **778768b4…**、作业 **27689**。三次单项 pytest 全部性能失败，正式 benchmark
> 中位数 **0.139725 ms**，仅 **2/8** 样本低于 0.139100 ms；采集本身正常。
> 同一 cubin 在历史 dadf9f3b… 上明显更快。本轮已完成复现，不需重复来确认失败；
> 用户补充当前只能使用一张 GPU；重新申请同卡不是换卡对照。下一步使用下述
> 单卡提交方式与负载状态诊断，详见 [验证记录](B300_VALIDATION.md)。
...
**尚未在 B300 验证 CUDA Graph 捕获兼容性或获取性能结果**；捕获失败保留 error.txt。
...
当前只是一项区分假设的诊断。默认负载块合计约 8 秒，无须再跑整套 62 项来
确认已知单项失败。根因尚未确定，若必须在目前较慢条件下满足原门槛，仍需在
