...
> 最新输入通知实验：`step10_ready.FAnkEQ` 已归档。候选 5/8 更快，但配对
> 耗时仅下降 0.049%，仍有 2/8 样本超过门槛，**不采用，稳定性问题未解决**。
> 此前三个写回候选也不采用。当前不需要原样重复这些实验或全量 pytest。

### 下一轮：输入分块粒度与缓冲容量，尚未在 GPU 测量

在服务器更新到包含 `run_step10_granularity.sh` 的版本后，只运行：

```bash
bash run_step10_granularity.sh
```

脚本比较正式 baseline 与三个独立构建，四个版本八轮位置平衡，保留原
warmup=10、repeat=30、seed=0 和 CUDA-event 计时；不用 Graph，不改变评分。
全部评分输出及八组边界先通过数值验证，才开始计时。每次建立新目录
`results_b300/step10_granularity.XXXXXX`，保存版本、GPU UUID、编译产物及原始样本。
...
像把四大箱输入拆成八小箱：总量相同，小箱可能更早供计算使用，但搬运请求、
等待及完成通知也增多。第一个候选单独测布局变化；第二个比较等容量下的粒度；
第三个只增加容量。K64/SW64 的 TMA 使用三维分块描述符，故第一个对照包含布局
所需的描述符变化；不能称为只改一位硬件参数。各版保留两组共享 B 的 MMA、
相同运算量、输出网格、双 TMEM 槽与 EPI32。仅原窄 N 双槽分支参与实验。

**如何决定下一步：**看 `paired_speedup` 对正式 baseline 的收益，`vs_control`
只用于归因；例如第三项胜过第二项但仍慢于 baseline，就不采用。八轮中至少
七轮更快、配对耗时降低约 3% 或更多，且八轮全部 ≤0.139100 ms，才值得进一步
独立复测；这些是筛选约定，不是统计显著性的证明，也不修改原评分。希望典型
耗时接近 ≤0.135 ms，并看最慢样本还有多少余量。若只快零点几个百分点或仍超线，
保留结果，不用反复刷全量 pytest。

若出现明显赢家，再只保留该候选和必要的直接对照复测两批，各 16 轮，覆盖不同
运行时段；然后检查四个评分尺寸、短 K/矩形边界，采用后跑完整 62 项 pytest
及全部 37 个评分形状。此顺序避免在候选尚未获益时重复完整验收。
脚本退出码 0 表示采集完成；summary 的 PASS 判断中位数，均不代表所有样本达标。
请保留整个结果目录，包括慢样本。当前没有该候选的 GPU 性能结论。
准备该实验的本地新增及受影响工具检查共 **248 passed，零跳过**，包括真实
SM100a/SM103a CUDA 生成；本机没有 B300，服务器数值与性能结果仍待回传。
