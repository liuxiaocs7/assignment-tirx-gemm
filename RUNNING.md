...
> 最新输入粒度实验 `step10_granularity.nVKp6r` 已归档：三个候选全部 0/8
> 更快，相对正式版配对耗时增加约 28%，**全部不采用**。正式版本轮仍有
> 2/8 样本超线，稳定性问题未解决；不用原样重复此实验或全量 pytest。

### 下一步：定位 SW128 / SW64 布局差异的成本

当前准备的是硬件对照，**还没有新的提速结果**。在已分配 B300 的终端运行一次：

```bash
uv run python -u profile_step10_layout.py
```

自动创建 `results_b300/step10_layout_profile.*`，按正式版、SW64、SW64、正式版
顺序串行采集四份 Nsight Compute 报告。每次独立编译、先验证输出、预热十次，
再采集一个 GEMM，采集后用重新写入的输出验算；编译和验算都在采集范围外。
SW64 使用上轮已经验证的 `tmem_k64_sw64`，保持 K64、四级 ring、共享 B 和原网格。
这是用已知慢版本帮助定位成本，不是重新筛选或采用它。

脚本查询当前 NCU 支持的指标，补采绝对周期/请求/字节数。`comparison.md`
列重点指标，`comparison.csv` 保留所有指标的四次原值、两组相邻配对比值和
同版本首尾比值；缺失指标明确显示 unavailable。比值统一是 SW64 / 正式版，
**不同计数器的增大有不同含义，不能统一读成“加速比”**。每份目录保存 raw.csv、
NCU 报告、CUDA/cubin、编译参数与 GPU UUID。程序核对相同设备/源码/工具设置、
重复同版本的编译产物；不相符或采集失败时保留记录并返回 1。

若 NCU 不在默认路径，可用 `--ncu /path/to/ncu`。采集时间受 replay 次数影响，
这四份不是四个评分样本，退出码 0 只表示诊断完成，不能证明性能达标。
下载后可在没有 GPU 的本机重建分析，指定新的输出目录：

```bash
python3 profile_step10_layout.py --analyze results_b300/step10_layout_profile.实际后缀 \
  --output /tmp/step10_layout_analysis_新目录
```
...
| 要区分的解释 | 本次观察 | 后续方向 |
|---|---|---|
| 计算取数/执行路径变贵 | 数学工作量固定时，Tensor Core 绝对活跃周期是否增加，两组配对是否一致 | 保留 SW128，优先隔离描述符与 MMA 发射调度；不继续扩大 SW64/K32 搜索 |
| TMA 搬运路径变贵 | TMA 请求、L2 sectors、DRAM bytes 与计算周期如何一起变化 | 单独控制 tensor map / 搬运布局，必要时补同口径角色插桩以测等待 |
| 控制指令或运行状态解释差异 | 动态指令数、时钟、同版本首尾指标是否变化 | 先检查生成代码和同版本漂移，不能把耗时差硬归因于内存布局 |

“活跃周期”类似机器开工的时间，不等于完成的工作量；取数更慢也可能让机器
更久处于活跃状态。TMA 请求数也不等于搬运延迟。`sm__mem_tensor` 是 **TMEM**
活动，不能当作 SMEM bank conflict；普通 LSU 指标也不能直接证明异步 MMA 的
SMEM 冲突。NCU replay 会扰动缓存和执行，四次对照只收集线索，不建立统计结论。
若计数器仍分不开原因，再做针对性的角色插桩或微基准。

实际提速仍须回到原 CUDA-event 计时，候选筛选目标为 4096 约≤0.135 ms、
平衡配对有一致收益、全部观测样本小于0.139100 ms，再独立复测和完整回归。
本地工具/生成代码检查 **51 passed**，包含两版生成的 CUDA kernel 主体与上轮
归档一致；此处没有本机 B300/NCU 实测，也未修改正式内核或评分计时。

### 已完成：输入分块粒度与缓冲容量，全部不采用

[原始样本](results_b300/step10_granularity.nVKp6r/step10_4096/samples.json)
对应执行版本 `c6a92ff`。四版本八轮位置平衡，原 warmup=10、repeat=30、seed=0。
评分形状与每候选八组边界数值验证均通过，三个候选却在每轮都明显慢于正式版。

| 配置 | 中位数 ms | 对正式版配对耗时增加 | 快于正式版 | 达标样本 |
|---|---:|---:|---:|---:|
| baseline：K64 / 四级 / SW128 | 0.138956 | — | — | 6/8 |
| tmem_k64_sw64 | 0.178131 | 28.155% | 0/8 | 0/8 |
| tmem_k32_depth8 | 0.178197 | 28.264% | 0/8 | 0/8 |
| tmem_k32_depth10 | 0.179044 | 28.716% | 0/8 | 0/8 |

最有区分力的结果是：K64/四级仅换 SW64 输入布局就已退化约28%；K32/八级
相对这个 SW64 对照基本持平（配对耗时增加0.108%，3/8更快），十级相对八级
又增加0.461%（1/8更快）。不能把整个退化归于 K32 请求增多。
四版 REG112、STACK/LOCAL0，没有寄存器溢出证据；布局改变也影响 TMA 描述符
及 MMA 取数方式，具体微观原因尚无逐候选 profile，详见 [验证记录](B300_VALIDATION.md)。

下面命令已经完成，仅保留用于复现，不是当前需要重复的下一步：

```bash
bash run_step10_granularity.sh
```

| 版本 | 每份输入 K / 缓冲级数 | A/B swizzle | 动态 SMEM 字节 | 直接对照 |
|---|---|---|---:|---|
| baseline | 64 / 4 | 128B | 181248 | — |
| tmem_k64_sw64 | 64 / 4 | 64B | 181248 | baseline |
| tmem_k32_depth8 | 32 / 8 | 64B | 181248 | tmem_k64_sw64 |
| tmem_k32_depth10 | 32 / 10 | 64B | 222208 | tmem_k32_depth8 |

实验像把四大箱输入拆成八小箱：容量相同不保证更高吞吐，小箱的布局、请求与
同步也有成本。三项结果均不足以进入采用，保留正式 K64/SW128 配置。
后续优化需要新的机制证据；本轮不继续扩大 SW64/K32 的级数搜索。

baseline 第1、5轮为0.139819733/0.139442126 ms，超过0.139100 ms门槛。
脚本退出码0表示采集完成，summary的PASS只判断中位数；不能从6/8推导稳定通过。
未来候选仍以约≤0.135 ms、同轮一致收益和全部观测样本达标为筛选目标，再
独立复测、检查四尺寸及边界，采用后执行完整62项pytest和37个评分形状。
本轮没有新全量验收。准备阶段248项本地检查已通过，GPU证据以本次归档为准。

### 已完成：Step 10 拆分输入就绪通知，基本持平

[原始样本](results_b300/step10_ready.FAnkEQ/step10_4096/samples.json)包含八轮
AB/BA 各四轮计时，原 warmup=10、repeat=30；评分形状及八组边界数值验证通过。

| 版本 | 中位数 ms | 最大值 ms | 更快轮数 | 达标样本 |
|---|---:|---:|---:|---:|
| baseline | 0.139018 | 0.139427 | — | 5/8 |
| tmem_split_ready | 0.138973 | 0.139199 | 5/8 | 6/8 |

候选的精确配对加速比为 **1.000491993×**，终端显示 1.000；典型配对节省
**0.068267 μs / 0.049167%**。两个中位数都 PASS，不能代表全部样本达标。
候选最慢轮仍超出 0.139100 ms 门槛约 0.099 μs，没有足够余量进入正式采用。
退出码 0 正确表示采集完整。详见 [验证记录](B300_VALIDATION.md)。

实验让每组分别等待共享 B 和自己的 A，保留输入总字节、四级 ring、EPI32、
原网格和 TMEM 槽位；输入释放仍要求两组 MMA 完成。新增通知也有代价，实测
没有得到有用的净收益。REG112/STACK0/LOCAL0 和动态 SMEM181248 B 不变。
编译器将 MMA 循环展开因子从四改成二，运算量仍相同，不能将小差异直接归因
于等待成本下降。

脚本及候选继续保留供复现；下列是**已完成的命令**：

```bash
bash run_step10_ready.sh
```

候选只作用于当前窄 N 双槽分支，其余形状保留原 CUDA 与 TMA 描述符。
验证边界为 `(4096,3072,K)` 的 K64/128/192/256/320/384、`(4608,2560,320)`、
`(512,9728,192)`，各两次。源码、编译产物、资源、版本/设备元数据和全部样本
保存在 `results_b300/step10_ready.FAnkEQ`。

准备该实验时本地新增及受影响工具回归 **207 passed，零跳过**；本轮分析
未重跑 GPU，也没有将候选合入正式内核。仍以原计时下约 ≤0.135 ms 为余量
目标，后续需要新的软件改动实测，不能凭本次 summary 的 PASS 宣称稳定修复。

### 已完成：写回候选全部不采用

[原始样本](results_b300/step10_epilogue.xVWC2d/step10_4096/samples.json)中，
三个候选的数值与六组边界均通过。八轮位置平衡计时结果：

| 配置 | 中位数 ms | 对 baseline 配对耗时增加 | 更快轮数 | 达标样本 |
|---|---:|---:|---:|---:|
| baseline | 0.139205 | — | — | 2/8 |
| 两次 64 列写回 | 0.141452 | 1.614% | 0/8 | 0/8 |
| 一次 128 列写回 | 0.143390 | 2.978% | 0/8 | 0/8 |
| 32 列 SMEM 双缓冲 | 0.140787 | 1.120% | 0/8 | 0/8 |

`run_step10_epilogue.sh` 保留供复现。三项已测未采用，不必重复。
加宽方案 REG 从 112 降到 105/100、没有栈或 LOCAL，仍然变慢；不能将它们
解释为寄存器溢出。实际静态写回指令从 4 减为 2/1，也不足以带来总耗时下降。
原因与证据边界见 [B300_VALIDATION.md](B300_VALIDATION.md)。

### 只有一张 GPU：提交方式与负载状态对照

用户回传 `step10_diagnose.9pEDv4`：仍为作业 27689、UUID 778768b4…，
三次单项 pytest 为 0.140401 / 0.140404 / 0.140261 ms，benchmark 中位数
0.139605 ms、2/8 样本达标。该目录现已由 `5da9d1f` 归档核对；重复失败已
充分复现。实际 CPU affinity 为 210–239，共 30 个逻辑 CPU，不能从
`CPUs/Task=1` 推断只分配了一个 CPU。当前不要仅为了确认失败重新申请同卡。

以下入口已在 `6ad3f6d` 成功运行，结果位于
[step10_runtime._v5qw220](results_b300/step10_runtime._v5qw220/)，保留命令供之后
设备策略变化时受控复测，当前不用原样重跑：

```bash
uv run python -u probe_step10_runtime.py
```

自动生成 `results_b300/step10_runtime.*`，保存版本/UUID/CPU 绑定、同一份正式
内核的 CUDA/cubin、逐轮计时与数值校验状态、负载时间区间、设备状态和退出码。
不改 GPU 映射、时钟、正式 kernel、pytest、计时器或门槛。本机相关 CPU 工具测试
**52 passed**（新诊断、已有聚焦入口与 benchmark CLI），
本次 B300 捕获/两次污染输出后的 replay 及 32 个计时输出校验均成功。
普通路径前/后中位数 **0.139833 / 0.139888 ms**，Graph 为
**0.136542 / 0.136804 ms**；退出码 1 反映普通路径仍有慢样本。

实验有三个阶段：

1. 普通 Python 提交与 CUDA Graph 提交交替测八轮。普通路径调用原计时器，
   warmup=10/repeat=30；Graph 路径也先普通预热 10 次，再一次提交包含 30 个
   相同 kernel 的图，按 30 归一化。输入、输出与编译产物相同。捕获通过 TVM FFI
   显式对齐 Torch 流；先将输出写成 NaN，重复 replay 并验算，以排除空图读到旧结果。
2. 单独运行普通→Graph→Graph→普通四个持续负载块，各约 2 秒；仅在此阶段每
   100 ms 采集实际分配 UUID 的 SM/显存时钟、功耗、温度和利用率。前后 `-q`
   保留限制累计计数。采样不能解析每个约 0.14 ms kernel 的瞬时状态。
3. 关闭监控后，再做八轮提交方式对照，记录持续负载前后的变化。

`summary.json` 的 `eager` 是普通提交，`graph` 是图提交。退出码 1 表示采集完成
但存在普通提交慢样本；2 表示采集/数值校验失败；0 也不替代原全量验收。
Graph 不报告评分 PASS，持续负载结果也不参与评分。不能把 Graph 的改善直接
当成纯 Python 开销：它也改变提交密度，可能影响 boost/缓存/调度状态。

| 观察 | 支持的下一步 |
|---|---|
| 普通约 0.140 ms，Graph 明显较快 | 提交间隙或提交密度相关状态值得查；用 GPU 时间线确认，不能直接改评分计时器 |
| 两种提交都约 0.140 ms，持续负载前后也相近 | CPU 逐次提交较难解释主要差距；结合负载时钟/限制计数，检查设备策略或继续优化内核 |
| 持续负载后普通提交也明显变化 | 有状态依赖信号；结合负载区间和遥测检查，不能只留下较快阶段 |

这项诊断已完成。80 次负载采样全为 SM 1095 MHz，77 次利用率为 99%–100%；
前后 SW Power Capping 新增 8.161490 s，温度最高 52°C、热限制计数未增加。
这提供了与管理员核对运行时功率管理、锁频/boost 与 power smoothing 策略的依据；
用户当前选择直接推进软件优化，上述核对不作为前置条件。
这不是已经证明人为锁频，也不能由显示的 1100 W 上限排除瞬时限制。完整数字
与边界见验证记录。无须再跑整套 62 项来确认已知失败，根因尚未确定；
若必须在目前较慢条件下满足原门槛，仍需在
原计时协议下验证优化后的余量；五级流水线约 0.31% 信号不足以预先承诺解决。

### Step 10 性能不稳定时的聚焦复现

以下入口保留用于后续确认修复效果，或在确实有其他设备时使用相同代码与参数对照。
本次 MBawzh 已运行完成。需要对照时，在仓库根目录执行：
...
MBawzh 已核对核心源码和历史编译产物相同。前三次 pytest 期间功耗/热限制
累计计数不变，benchmark 窗口功耗限制增加 178.190 ms，热限制仍未增加；
不能据此把所有失败归于限频。1095 MHz 是进程结束后的快照值，运行时频率未知。
同卡不同作业的失败说明单纯重分配作业没有消除问题；跨卡比较仍需记录 CPU
配额、绑定与节点，避免把宿主差异当作 GPU 差异。不要手动覆盖 Slurm 的 GPU 映射。
...
**`9e3b989` 五轮全量 pytest 均为 62 passed，五份正式全步骤 benchmark 共
185/185 PASS，当前实现可记录为本次 B300 验收通过。** 这也补齐了 Step 9
缓存采用后的回归。每次 benchmark 为 trials=1、warmup=10、repeat=30；
五次完整运行不能描述为每尺寸七轮。五份 CSV 的版本和四个核心源码哈希已核对，
内核/计时器/评分测试从该版本至 `f06d88f` 未改变。

Step 10／4096 的五次正式 benchmark 为 0.094261–0.094725 ms，最慢仍有
31.90% 余量；8192 为 0.642770–0.649407 ms。五轮 pytest 与 CSV 的完整索引、
版本归属及缺失元数据的范围见 [B300_VALIDATION.md](B300_VALIDATION.md)。
历史全量 61 passed / 1 failed（4096 为 0.140561 ms）及历史慢样本继续保留，
新验收不表示所有 GPU 分配都相同。此前 57 项全量与 Step 10 的 28/28 样本
验收也保留在 [step10_final.vM3h3I](results_b300/step10_final.vM3h3I/)。

几何实验 `step10_geometry.PywrmG` 已完成，三个候选均 8/8 慢于 baseline，
不采用 74 clusters 或 N64。较小 L2 分组已排除；N128 五级虽约 0.31% 收益，
仍有超线样本而保留为候选。正式 Step 10 维持共享 B、N128 双槽、四级输入、
...
当前全量为 62 项；`9e3b989` 已有五次独立的 62 passed 日志，包含上述五项。
以下命令仅供重放：
...
以下是历史正式 Step 10 benchmark 汇总，四个尺寸均为 7/7 样本达标。
...
下面命令用于后续内核改动后的验收或独立复现；当前版本已通过五轮全量回归，
无需为了本次验收重复此处流程或 `run_step9_validate.sh`。
...
**缓存已移植到正式 Step 9，`9e3b989` 五轮全量 pytest 与全步骤 benchmark 均通过。**
...
这套带编译产物的七轮流程与已完成的五次 trials=1 回归分开记录；保留失败或
超线结果，不改变原评分门槛。
...
### 已完成：Step 10 网格与 N64 tile 对照，三候选不采用

`step10_geometry.PywrmG` 八轮结果已核对：三个候选都 0/8 快于 baseline，
对 baseline 配对耗时分别增加约 4.75% / 36.50% / 24.22%。四个主构建与
48 次边界启动数值验证通过；N64 四级只有 3/8 样本达标，其他版本为 8/8。
N64 五级对 N64 四级的 1.099× 改善仍不足以超过正式内核，三个候选均不采用。
本轮已完成，无需重复，以下命令仅供重放：

```bash
bash run_step10_geometry.sh
```

结果保存在 `results_b300/step10_geometry.XXXXXX`。本轮只测 4096 的四版本，
warmup=10、repeat=30、trials=8，顺序为 ABCD/DCBA/CDAB/BADC 重复两次。
每个版本在每个位置出现两次，各对版本先后次数相等。自动补齐的直接对照为：

| 版本 | 唯一新增因素 | 直接对照 | 4096 任务数 / cluster 数 | 动态 SMEM B/CTA |
|---|---|---|---:|---:|
| `baseline` | 当前 N128、四级、均衡网格 | — | 256 / 64 | 181248 |
| `tmem_max_clusters` | 允许使用全部 74 个 cluster | baseline | 256 / 74 | 181248 |
| `tmem_n64` | MMA N128→64；每 CTA 的 B 行数 64→32 | tmem_max_clusters | 512 / 74 | 164864 |
| `tmem_n64_depth5` | N64 输入四级→五级 | tmem_n64 | 512 / 74 | 201728 |

两个 consumer 仍共享 B，保留 K64、EPI32、group8、两个 TMEM 槽及 512 列分配。
用**原 N128 配置的双槽判定**限定候选范围；1024/2048 单槽、8192 宽 N、
`(512,9472,192)` 单波边界与 `(4096,4352,320)` 宽 N 路径的生成代码和
TMA 描述符保持一致。新实验放在 `probe_step10_geometry.py`，正式内核不变。

计时前，四个方阵构建均先验算；三个候选各验证 `(4096,3072,K)` 的
K=64/128/192/256/320/384、`(4608,2560,320)` 部分 L2 分组和
`(512,9728,192)` 双槽边界，每个形状两次，共 **48 次边界启动**。
计时后逐轮验算输出。保存源码哈希、builder/diff、CUDA/cubin、资源报告、
GPU 前后快照及完整顺序/样本；监控与编译回调不进入计时窗口。

N64 的假设是更细任务和更高网格利用率能减少尾部空闲；代价是任务/MMA 次数
翻倍、总 TMA 请求字节约为 N128 的 1.8 倍，不保证更快。旧 N256 单槽下已经
试过网格均衡，不能把这轮 N128 双槽的 74-cluster 对照说成从未尝试的方向。
五级只作为新 N64 几何的直接对照，不重跑已完成的 N128 五级实验。

本次 baseline 中位数/最大值为 0.109001/0.111588 ms，已经满足原先筛选的
≤0.135 ms 目标；不能因为 N64 五级 PASS 就认为它更优。判定仍以直接对照、
候选先后位置与逐轮样本为准，原评分门槛 **0.139100 ms** 不变。退出码 0 表示
probe 正常完成，SLOW 须保留；正式验收另由前述 `9e3b989` 五轮全量记录支持。
...
两次均使用修复前的顺序生成器，双版本复测始终 baseline 先测。随后
`step10_depth5_balanced.Fm3CcJ` 已用八轮 AB/BA 完成复核：配对加速 **1.003139×**、
8/8 更快，AB/BA 分别为 1.002920× / 1.003622×；小幅收益得到支持，但五级
第 1、2 轮仍超线约 0.222/0.150 μs，只有 6/8 达标。原始样本、源码哈希和
编译产物已核对；五级暂保留为候选，不将中位数 PASS 写为稳定达标。
无需重复，以下脚本保留供重放，保持 L2 group=8、共享 B、K64 和双 TMEM 槽：

```bash
bash run_step10_depth5.sh
```

脚本用八轮 AB/BA（各四次），每轮仍是 warmup=10、repeat=30；记录版本/
内核哈希、GPU UUID、前后状态、原始样本、编译产物和 probe/tee 退出码。
输出为 `results_b300/step10_depth5_balanced.XXXXXX`。计时前验证 4096 及
五级六组短 K 边界，每组两次。这轮已完成，不因两个中位数 PASS 忽略超线
样本。无需重新跑 L2 或 201 轮共享 A 实验。

以下旧命令仅供历史重放，平衡顺序复核使用上面的八轮脚本：
...
完全一致，小幅正向信号重复出现但受旧顺序影响，第二次有三个样本超线。正式内核、门槛和
...
### 已完成：窄 tile 的 L2 分组对照，三候选不采用

`9ad4a38` 的 `step10_l2.nqNoLW` 已回传：四个构建和 36 次边界启动验证
通过，group4/2/1 却分别只有 0/0/2 轮快于 baseline，中位数全部 SLOW。
baseline 本身 3/8 达标，性能稳定性仍未解决。保留 group=8，不重复这组实验。
原目录已同步到本地，原始样本复核结论不变；下列命令仅供重放：

```bash
bash run_step10_l2.sh
```

脚本比较 production baseline（group=8）与 `tmem_l2_group4/2/1`，固定
4096、八轮、warmup=10、repeat=30、seed=0。四版本的测量位置和两两先后
次数在八轮内平衡；各候选都直接与 baseline 配对，不叠加输入环或共享 A 改动。
只在窄 N／双 TMEM 槽路径调整 tile 调度顺序，正式内核保持不变。

计时前验证四个版本的 4096 方阵及候选各六组边界，每组边界连续运行两次并重置 NaN。
边界包括 K64/192/256/320 的跨 tile 重用、九行 tile 的部分 L2 分组、单波
临界点上方 `(512,9728,192)`。本地另核对小尺寸、宽 N 和单槽分支代码不变。
结果保存到 `results_b300/step10_l2.XXXXXX`，包含原始顺序、逐轮样本、CUDA/
cubin、版本、GPU UUID 和前后快照；快照不证明计时期间状态恒定。

请保留整个目录。先看候选相对 baseline 的逐轮收益及最慢样本是否越过
0.139100 ms，再判断是否值得采用；`SLOW` 不作为工具错误，退出码 0 也不代表
性能通过。八轮只是筛选，不能证明所有运行条件下稳定；有明确收益后再做正式
全量验收。旧 wide-N 的 `l2_group_4` 曾略慢，本轮用不同名称区分当前窄 tile。
