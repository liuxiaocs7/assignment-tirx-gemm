...
> 最新写回实验（2026-09-15）：`step10_epilogue.xVWC2d` 的三个候选全部
> 0/8 快于 baseline，配对耗时增加 1.12%–2.98%，不采用。正式 baseline
> 本轮仅 2/8 样本达标，软件性能稳定性仍待改进。详见 [验证记录](B300_VALIDATION.md)。

### 当前要运行：Step 10 拆分输入就绪通知
...
每个计算组只需要共享 B 和自己的 A。原版让两组共同等 B/A0/A1 全部到齐；
`tmem_split_ready` 分开通知 B、A0、A1，让每组等待它真正需要的两份数据。
输入缓冲依然等两组计算都完成才复用，B 仍只加载一次。**尚未测量这个候选，
额外通知可能抵消收益，不能预先承诺提速。**

| 项目 | 正式 baseline | tmem_split_ready |
|---|---|---|
| K64 阶段就绪通知 | 一个，81920 B | B 16384 B；A0/A1 各 32768 B |
| 每 consumer 的 acquire | 等全部输入 | 等 B 和自己的 A |
| 输入槽释放 | 两 consumer 都计算完 | 相同 |
| 输入深度 / 动态 SMEM | 4 / 181248 B | 相同 |
| 写回 / 网格 / TMEM 槽位 | EPI32 / 64 clusters / 双槽（4096） | 相同 |

仅在当前窄 N 双槽分支启用。小尺寸单槽及宽 N 分支保留原生成 CUDA 和
TMA 描述符。新增 barrier 放入原先预留区域，数据缓冲和原有 barrier 地址不动。

脚本先检查两个评分构建；候选在 `(4096,3072,K)` 的 K64/128/192/256/320/384、
`(4608,2560,320)`、`(512,9728,192)` 八个边界各验算两次。随后原 CUDA-event
计时器以 warmup=10/repeat=30 测八轮，baseline/候选按 AB/BA 各四轮。
所有原始结果保存于 `results_b300/step10_ready.*`，编译、数值、管道或采集失败
保留非零状态；退出码 0 只表示采集完整，性能看原始样本与 summary。

本地工具检查可验证地址、字节计数、等待/复用顺序及回退分支，不能执行 B300
数值和计时。本次新增及受影响回归 **207 passed，零跳过**。
原评分阈值 0.139100 ms 保持，优化目标约 ≤0.135 ms 留余量。
本次不需要原样重跑失败的写回、L2、几何实验或全量 pytest；出现有余量的正向
结果后才进入采用及正式全量回归。
