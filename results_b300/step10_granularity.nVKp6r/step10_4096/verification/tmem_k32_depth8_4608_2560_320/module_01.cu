...
extern "C" __global__ void __launch_bounds__(384) kernel_kernel(const __grid_constant__ CUtensorMap A_tensormap, const __grid_constant__ CUtensorMap B_tensormap, const __grid_constant__ CUtensorMap D_tensormap) {
  ...
  tvm_builtin_ptx_tcgen05_encode_matrix_descriptor((&(descB_ptr[0])), (&(((half*)pool_buf_ptr)[66048])), 0, 32, 2);
  ...
  if (tvm_builtin_cuda_thread_rank() == 0) {
    tvm_builtin_ptx_mbarrier_init((&(((uint64_t*)pool_buf_ptr)[17])), 1);
    tvm_builtin_ptx_mbarrier_init((&(((uint64_t*)pool_buf_ptr)[18])), 1);
    tvm_builtin_ptx_mbarrier_init((&(((uint64_t*)pool_buf_ptr)[19])), 1);
    tvm_builtin_ptx_mbarrier_init((&(((uint64_t*)pool_buf_ptr)[20])), 1);
  }
  if (tvm_builtin_cuda_thread_rank() == 0) {
    tvm_builtin_ptx_mbarrier_init((&(((uint64_t*)pool_buf_ptr)[21])), 256);
    tvm_builtin_ptx_mbarrier_init((&(((uint64_t*)pool_buf_ptr)[22])), 256);
    tvm_builtin_ptx_mbarrier_init((&(((uint64_t*)pool_buf_ptr)[23])), 256);
    tvm_builtin_ptx_mbarrier_init((&(((uint64_t*)pool_buf_ptr)[24])), 256);
  }
  ...
  if ((warp_id_in_cta >> 2) == 2) {
    if ((warp_id_in_cta & 3) == 3) {
      if (tvm_builtin_elect_one_sync_op() != (uint)0) {
        ...
        while (1) {
          ...
          for (int k = 0; k < 10; ++k) {
            tvm_builtin_ptx_mbarrier_try_wait((&(((uint64_t*)pool_buf_ptr)[(tma_phase_stage_ptr[0] + 9)])), (tma_phase_phase_ptr[0] ^ 0));
            int cse_v6 = (k * 32);
            ptx_cp_async_bulk_tensor_g2s_cluster_tile_2d_mbar_addr((&(((half*)pool_buf_ptr)[((tma_phase_stage_ptr[0] * 2048) + 66048)])), tvm_builtin_cuda_cvta_generic_to_shared((&(remote_mbar_ptr[tma_phase_stage_ptr[0]]))), ((unsigned long long)(&(B_tensormap))), 0, (uint64_t)0, (k * 32), ((_ptr_1[0] * 128) + (((int)tvm_builtin_cluster_ctaid_x()) * 64)));
      ...
      if ((warp_id_in_cta & 3) < 2) {
        if (((int)tvm_builtin_cluster_ctaid_x()) == 0) {
          if (tvm_builtin_elect_one_sync_op() != (uint)0) {
            ...
            while (1) {
              if (!((_ptr_2[0] < 180))) { break; }
              tvm_builtin_ptx_mbarrier_try_wait((&(((uint64_t*)pool_buf_ptr)[(((ld_phase_stage_ptr[0] * 2) + (warp_id_in_cta & 3)) + 21)])), (ld_phase_phase_ptr[0] ^ 0));
              for (int k_1 = 0; k_1 < 10; ++k_1) {
                ...
                ptx_tcgen05_commit_cta_group_2_multicast((&(((uint64_t*)pool_buf_ptr)[(mma_phase_stage_ptr[0] + 9)])), 3);
                mma_phase_stage_ptr[0] = (mma_phase_stage_ptr[0] + 1);
                if (mma_phase_stage_ptr[0] == 8) {
                  mma_phase_stage_ptr[0] = 0;
                  mma_phase_phase_ptr[0] = (mma_phase_phase_ptr[0] ^ 1);
                }
              }
              ptx_tcgen05_commit_cta_group_2_multicast((&(((uint64_t*)pool_buf_ptr)[(((ld_phase_stage_ptr[0] * 2) + (warp_id_in_cta & 3)) + 17)])), 3);
    ...
    if (warp_id_in_cta < 8) {
      ...
      while (1) {
        if (!((_ptr_2[0] < 180))) { break; }
        tvm_builtin_ptx_mbarrier_try_wait((&(((uint64_t*)pool_buf_ptr)[(((wb_phase_stage_ptr[0] * 2) + (warp_id_in_cta >> 2)) + 17)])), (wb_phase_phase_ptr[0] ^ 0));
        tvm_builtin_ptx_tcgen05_fence_after_thread_sync();
        ...
        tvm_builtin_ptx_mbarrier_arrive_shared_cluster_remote_pred((&(((uint64_t*)pool_buf_ptr)[(((wb_phase_stage_ptr[0] * 2) + (warp_id_in_cta >> 2)) + 21)])), 0, actual_pred_ptr_1[0]);
        ...
        int cse_v30 = (((warp_id_in_cta >> 2) * 4096) + 82432);
