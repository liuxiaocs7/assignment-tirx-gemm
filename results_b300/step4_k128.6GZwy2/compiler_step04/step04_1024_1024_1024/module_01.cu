...
extern "C" __global__ void __launch_bounds__(128) kernel_kernel(const __grid_constant__ CUtensorMap A_tensormap, const __grid_constant__ CUtensorMap B_tensormap, const __grid_constant__ CUtensorMap D_tensormap) {
  ...
  tvm_builtin_ptx_tcgen05_encode_matrix_descriptor((&(descB_ptr[0])), (&(((half*)pool_buf_ptr)[16896])), 1024, 64, 3);
  ...
  if ((warp_id_in_cta % 4) == 0) {
    if (tvm_builtin_elect_one_sync_op() != (uint)0) {
      for (int k = 0; k < 8; ++k) {
        int cse_v4 = (k * 2);
        ptx_cp_async_bulk_tensor_g2s_cluster_tile_3d((&(((half*)pool_buf_ptr)[512])), (&(((uint64_t*)pool_buf_ptr)[1])), ((unsigned long long)(&(A_tensormap))), 0, (uint64_t)0, 0, (((int)blockIdx.x) * 128), (k * 2));
        ptx_cp_async_bulk_tensor_g2s_cluster_tile_3d((&(((half*)pool_buf_ptr)[16896])), (&(((uint64_t*)pool_buf_ptr)[1])), ((unsigned long long)(&(B_tensormap))), 0, (uint64_t)0, 0, (((int)blockIdx.y) * 128), (k * 2));
