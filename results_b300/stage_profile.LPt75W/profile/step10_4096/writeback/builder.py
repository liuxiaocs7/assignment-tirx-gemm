def hgemm_v10(M, N, K):
    ...
    def kernel(
        ...
        Profile: T.Buffer((148, 2, 2, 11), "int64"),
    ):
        # fmt: off
        T.device_entry()
        ...
        if wg_id == 2:
            ...
            while tile_scheduler.valid():
                T.cuda.func_call("tirx_profile_start", profile_state.data, (warp_id == 0) & (lane_id == 0), source_code=PROFILE_SOURCE)
                T.cuda.func_call("tirx_profile_mark", profile_state.data, (warp_id == 0) & (lane_id == 0), source_code=PROFILE_SOURCE)
                mma2ld.wait(wg_id, wb_phase.phase)
                T.cuda.func_call("tirx_profile_add_wait", profile_state.data, (warp_id == 0) & (lane_id == 0), source_code=PROFILE_SOURCE)
                wb_phase.advance()
                T.cuda.func_call("tirx_profile_mark", profile_state.data, (warp_id == 0) & (lane_id == 0), source_code=PROFILE_SOURCE)
                T.ptx.tcgen05.fence.after_thread_sync()
                ...
                T.cuda.func_call("tirx_profile_add_work", profile_state.data, (warp_id == 0) & (lane_id == 0), source_code=PROFILE_SOURCE)
                T.cuda.func_call("tirx_profile_mark", profile_state.data, (warp_id == 0) & (lane_id == 0), source_code=PROFILE_SOURCE)
                for i in T.unroll(MMA_N // EPI_N):
                    ...
                    T.cuda.warpgroup_sync(wg_id + 10)
                T.cuda.func_call("tirx_profile_add_epilogue", profile_state.data, (warp_id == 0) & (lane_id == 0), source_code=PROFILE_SOURCE)
                T.cuda.func_call("tirx_profile_finish", profile_state.data, Profile.ptr_to([bx, tile_scheduler.tile_count, wg_id, 0]), tile_scheduler.m_idx, tile_scheduler.n_idx, (warp_id == 0) & (lane_id == 0), source_code=PROFILE_SOURCE)
                tile_scheduler.next_tile()
