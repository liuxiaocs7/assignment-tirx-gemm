    ...
    if ((warp_id_in_cta & 3) == 3) {
      if (tvm_builtin_elect_one_sync_op() != (uint)0) {
        ...
        while (1) {
          ...
          ptx_cp_async_bulk_tensor_g2s_cluster_tile_2d_mbar_addr((&(((half*)pool_buf_ptr)[((tma_phase_stage_ptr[0] * 16384) + 512)])), tvm_builtin_cuda_cvta_generic_to_shared((&(remote_mbar_ptr_1[(tma_phase_stage_ptr[0] * 2)]))), ((unsigned long long)(&(A_tensormap))), 0, (uint64_t)0, 0, ((_ptr[0] * 512) + (((int)tvm_builtin_cluster_ctaid_x()) * 128)));
          ptx_cp_async_bulk_tensor_g2s_cluster_tile_2d_mbar_addr((&(((half*)pool_buf_ptr)[((tma_phase_stage_ptr[0] * 16384) + 8704)])), tvm_builtin_cuda_cvta_generic_to_shared((&(remote_mbar_ptr_1[((tma_phase_stage_ptr[0] * 2) + 1)]))), ((unsigned long long)(&(A_tensormap))), 0, (uint64_t)0, 0, (((_ptr[0] * 512) + (((int)tvm_builtin_cluster_ctaid_x()) * 128)) + 256));
