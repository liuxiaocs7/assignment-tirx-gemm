  ...
  if ((warp_id_in_cta >> 2) == 2) {
    ...
    if (warp_id_in_cta < 8) {
      ...
      while (1) {
        ...
        tirx_profile_finish(profile_state_ptr, (&(Profile_ptr[(((((int)blockIdx.x) * 88) + (tile_scheduler_tile_count_ptr[0] * 22)) + ((warp_id_in_cta >> 2) * 11))])), _ptr[0], _ptr_1[0], (((warp_id_in_cta % 4) == 0) & ((((int)threadIdx.x) % 32) == 0)));
        _ptr_2[0] = (_ptr_2[0] + 64);
        tile_scheduler_tile_count_ptr[0] = (tile_scheduler_tile_count_ptr[0] + 1);
        if ((bool)1 & (_ptr_2[0] < 256)) {
