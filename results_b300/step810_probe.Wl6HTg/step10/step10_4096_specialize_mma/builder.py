                ...
                for mma_consumer in T.unroll(NUM_CONSUMER):
                    if warp_id == mma_consumer:
                        if cbx == 0:
                            if T.filter(lane_id, T.ptx.elect_sync()):
                                while tile_scheduler.valid():
                                    ld2mma.wait(mma_consumer, ld_phase.phase)
                                    ld_phase.advance()
                                    for k in range(K_TILES):
                                        tma2mma.wait(mma_phase.stage, mma_phase.phase)
                                        T.ptx.tcgen05.fence.after_thread_sync()
                                        Tx.gemm_async(tmem[:, mma_consumer * MMA_N:(mma_consumer + 1) * MMA_N],
                                            Asmem[mma_phase.stage, mma_consumer, :, :], Bsmem[mma_phase.stage, :, :],
                                            accum=(k != 0), dispatch="tcgen05", cta_group=CTA_GROUP)
                                        mma2tma.arrive(mma_phase.stage, cta_group=CTA_GROUP, cta_mask=3)
                                        mma_phase.advance()
                                    mma2ld.arrive(mma_consumer, cta_group=CTA_GROUP, cta_mask=3)
                                    tile_scheduler.next_tile()
