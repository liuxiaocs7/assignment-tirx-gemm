  ...
  if ((warp_id_in_cta >> 2) == 2) {
      ...
      if (tvm_builtin_elect_one_sync_op() != (uint)0) {
        ...
        while (1) {
          ...
          for (int k = 0; k < 6; ++k) {
            ...
            int cse_v6 = (k * 64);
            ptx_cp_async_bulk_tensor_g2s_cluster_tile_2d_mbar_addr((&(((half*)pool_buf_ptr)[((tma_phase_stage_ptr[0] * 4096) + 33280)])), tvm_builtin_cuda_cvta_generic_to_shared((&(remote_mbar_ptr[tma_phase_stage_ptr[0]]))), ((unsigned long long)(&(B_tensormap))), 0, (uint64_t)0, (k * 64), ((_ptr_1[0] * 128) + (((int)tvm_builtin_cluster_ctaid_x()) * 64)));
    ...
    if (warp_id_in_cta < 8) {
      ...
      while (1) {
        ...
        int cse_v29 = (((warp_id_in_cta >> 2) * 4096) + 41472);
