def hgemm_v10(M, N, K):
    ...
    PIPE_DEPTH = 6 if SHARE_A else 4
    ...
    A_layout = mma_shared_layout(a_type, SwizzleMode.SWIZZLE_128B_ATOM, (PIPE_DEPTH, A_CONSUMERS, BLK_M, BLK_K))
    B_layout = mma_shared_layout(b_type, SwizzleMode.SWIZZLE_128B_ATOM, (PIPE_DEPTH, B_CONSUMERS, BLK_N, BLK_K))
    ...
    def kernel(
        ...
        Asmem = pool.alloc((PIPE_DEPTH, A_CONSUMERS, BLK_M, BLK_K), a_type, layout=A_layout)
        Bsmem = pool.alloc((PIPE_DEPTH, B_CONSUMERS, BLK_N, BLK_K), b_type, layout=B_layout)
        ...
        @T.inline
        def tma_load(k_st):
            for consumer in T.unroll(B_CONSUMERS):
        ...
        if wg_id == 2:
                        ...
                        while tile_scheduler.valid():
                            ...
                            for k in range(K_TILES):
                                ...
                                Tx.gemm_async(tmem[:, (ld_phase.stage * TMEM_SLOT_STRIDE + warp_id) * MMA_N:(ld_phase.stage * TMEM_SLOT_STRIDE + warp_id + 1) * MMA_N],
                                    Asmem[mma_phase.stage, 0 if SHARE_A else warp_id, :, :], Bsmem[mma_phase.stage, warp_id if SHARE_A else 0, :, :],
            ...
            while tile_scheduler.valid():
                ...
                for i in T.unroll(MMA_N // EPI_N):
                    ...
                    if warp_id == 0:
                        if T.filter(lane_id, T.ptx.elect_sync()):
                            Tx.copy_async(D[m_out:m_out + BLK_M, n_consumer_out + col:n_consumer_out + col + EPI_N],
