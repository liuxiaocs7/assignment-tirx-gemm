def hgemm_v10(M, N, K):
    ...
    EPI_BUFFERS = 2 if TMEM_BUFFERS == 2 else 1
    ...
    D_layout = mma_shared_layout(d_type, D_SWIZZLE, (NUM_CONSUMER, EPI_BUFFERS, BLK_M, EPI_N))
    ...
    def kernel(
        ...
        Dsmem = pool.alloc((NUM_CONSUMER, EPI_BUFFERS, BLK_M, EPI_N), d_type, layout=D_layout)
        pool.commit()
        ...
        if wg_id == 2:
            ...
            while tile_scheduler.valid():
                ...
                for i in T.unroll(MMA_N // EPI_N):
                    ...
                    Tx.copy(Dsmem[wg_id, i % EPI_BUFFERS, warp_id * 32 + lane_id, :], Dreg_f16[col:col + EPI_N])
                    T.ptx.fence.proxy_async("shared::cta")
                    ...
                    if warp_id == 0:
                        if T.filter(lane_id, lane_id == 0 if EPI_BUFFERS == 2 else T.ptx.elect_sync()):
                            Tx.copy_async(D[m_out:m_out + BLK_M, n_out + col:n_out + col + EPI_N],
                                          Dsmem[wg_id, i % EPI_BUFFERS, :, :], dispatch="tma_auto")
