#!/usr/bin/env bash
# Recheck the existing depth-five candidate with four AB/BA pairs.
set -uo pipefail

if (( $# != 0 )); then
  printf 'Usage: bash run_step10_depth5.sh\n' >&2
  exit 2
fi
...
tirx_run=$(mktemp -d results_b300/step10_depth5_balanced.XXXXXX) || exit 1
printf '结果目录：%s\n' "$tirx_run"
{
  date -Iseconds
  hostname
  ...
  printf 'size=4096 input_depths=4,5 l2_group=8 trials=8 warmup=10 repeat=30 seed=0\n'
