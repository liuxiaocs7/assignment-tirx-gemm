#!/usr/bin/env bash
# Current shared-B kernel versus independent input-ready notifications; eight balanced trials.
set -uo pipefail

if (( $# != 0 )); then
  printf 'Usage: bash run_step10_ready.sh\n' >&2
  exit 2
fi
...
tirx_run=$(mktemp -d results_b300/step10_ready.XXXXXX) || exit 1
printf '结果目录：%s\n' "$tirx_run"
{
  ...
  printf 'size=4096 variants=baseline,tmem_split_ready trials=8 warmup=10 repeat=30 seed=0\n'
