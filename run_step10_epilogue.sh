#!/usr/bin/env bash
# Current shared-B kernel versus three output-staging candidates; eight balanced trials.
set -uo pipefail

if (( $# != 0 )); then
  printf 'Usage: bash run_step10_epilogue.sh\n' >&2
  exit 2
fi
...
tirx_run=$(mktemp -d results_b300/step10_epilogue.XXXXXX) || exit 1
printf '结果目录：%s\n' "$tirx_run"
{
  ...
  printf 'size=4096 variants=baseline,tmem_epi64,tmem_epi128,tmem_epi32_double trials=8 warmup=10 repeat=30 seed=0\n'
...
uv run python -u probe_persistent.py --steps 10 --size 4096 --trials 8 \
  ...
  --variants tmem_epi64 tmem_epi128 tmem_epi32_double \
