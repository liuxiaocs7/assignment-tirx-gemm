#!/usr/bin/env bash
# Independent L2 ordering probes; four variants, two balanced four-trial cycles.
set -uo pipefail

if (( $# != 0 )); then
  printf 'Usage: bash run_step10_l2.sh\n' >&2
  exit 2
fi
...
tirx_run=$(mktemp -d results_b300/step10_l2.XXXXXX) || exit 1
printf '结果目录：%s\n' "$tirx_run"
{
  date -Iseconds
  hostname
  ...
  printf 'size=4096 l2_groups=8,4,2,1 trials=8 warmup=10 repeat=30 seed=0\n'
...
uv run python -u probe_persistent.py --steps 10 --size 4096 --trials 8 \
  ...
  --variants tmem_l2_group4 tmem_l2_group2 tmem_l2_group1 \
