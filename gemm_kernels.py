...
def hgemm_v9(M, N, K):
    ...
    def kernel(
        ...
        with Tx.kernel():
            ...
            n_out = Tx.meta_var(tile_scheduler.n_idx * MMA_N)
    ...
    def kernel(
        ...
        with Tx.kernel():
            ...
            tile_scheduler = ClusterPersistentScheduler2D(
                ...
                l2_group_size=8, num_clusters=SM_COUNT // CTA_GROUP)
            tile_scheduler.init(bx // CTA_GROUP)
            m_st = Tx.meta_var(tile_scheduler.m_idx * MMA_M * NUM_CONSUMER + cbx * BLK_M)
            n_st = Tx.meta_var(tile_scheduler.n_idx * MMA_N + cbx * BLK_N)
            n_out = Tx.meta_var(tile_scheduler.n_idx * MMA_N)
            tma2mma_cta0 = tma2mma.remote_view(0)
            tma_phase = PipelineState("tma", PIPE_DEPTH)
            ...
            def tma_load(k_st):
                for consumer in Tx.unroll(NUM_CONSUMER):
                    m_consumer = Tx.meta_var(m_st + consumer * MMA_M)
            ...
            if wg_id == 2:
                ...
                m_out = Tx.meta_var(m_st + wg_id * MMA_M)
                Dreg = Tx.alloc_local((TMEM_LD_N,), acc_type)
                Dreg_f16 = Tx.alloc_local((MMA_N,), d_type)
                Dreg_wg = Dreg.view(128, TMEM_LD_N,
                    layout=TileLayout(S[(128, TMEM_LD_N) : (1@axis_tid_in_wg, 1)]))
                while tile_scheduler.valid():
                    ...
                    for i in Tx.unroll(MMA_N // TMEM_LD_N):
                        col = Tx.meta_var(i * TMEM_LD_N)
                        with Tx.warpgroup():
                            Tx.copy(Dreg_wg[:, :], tmem[:, wg_id * MMA_N + col:wg_id * MMA_N + col + TMEM_LD_N])
                        with Tx.thread():
                            Tx.cast(Dreg_f16[col:col + TMEM_LD_N], Dreg[:])
                    # All TMEM reads have finished. MMA can overlap the TMA epilogue.
                    with Tx.thread():
